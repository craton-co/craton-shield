...
fn modbus_tcp_max_pdu_length_no_panic() {
    ...
    // Build a max-length PDU through the validated API.
    let pdu = [0u8; 253];
    let frame = ModbusTcpFrame::with_pdu(
        0,
        0,
        0,
        vs_types_ind::ModbusFunctionCode::ReadHoldingRegisters,
        0x03,
        0,
        0,
        &pdu,
...
///
/// The validated API can no longer produce an overflow frame on its own; the
/// `__set_pdu_len_unchecked` helper simulates the ABI-level corruption that
/// would arrive from raw FFI input.
#[test]
fn modbus_tcp_pdu_len_overflow_handled_safely() {
    let mut frame = ModbusTcpFrame::default();
    frame.__set_pdu_len_unchecked(255); // Exceeds MAX_MODBUS_PDU_LEN (253)
    assert!(
...
fn modbus_tcp_zero_pdu_length_no_panic() {
    let mut mon = ModbusMonitor::new();
    let frame = ModbusTcpFrame::default(); // default pdu is empty
    let _ = mon.inspect_tcp(&frame);
}
...
fn modbus_rtu_max_pdu_length_no_panic() {
    let mut mon = ModbusMonitor::new();
    let pdu = [0u8; 253];
    let frame = ModbusRtuFrame::with_pdu(
        0,
        vs_types_ind::ModbusFunctionCode::ReadHoldingRegisters,
        0x03,
        0,
...
fn modbus_rtu_pdu_len_overflow_handled_safely() {
    let mut frame = ModbusRtuFrame::default();
    frame.__set_pdu_len_unchecked(255);
