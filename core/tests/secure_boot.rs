...
use vs_secure_boot::{
    BootEntry, BootFailurePolicy, BootStage, BootVerificationOutcome, BootVerifier, HardwareTpm,
    TpmAttestation,
};
use vs_types::KeyId;
...
#[test]
fn hardware_tpm_quote_verify_round_trip() {
...
#[test]
fn hardware_tpm_multiple_pcr_extension() {
    let crypto = make_crypto();
    let mut tpm = HardwareTpm::new(crypto, KeyId(0));
