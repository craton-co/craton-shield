...
//!
//! ```toml
//! vs-crypto = { version = "0.6", features = ["pq"] }
//! ```
//!
//! # Key storage
//!
...
///
/// The no-op RNG means that `set_mlkem_key(slot, None)` / `set_mldsa_key(slot, None)`
/// (random-seed variants) would produce all-zero seeds — this is intentionally
/// insecure and should never be used in production.  Always provision keys via
/// [`PostQuantumProvider::provision_mlkem_key`] or
/// [`PostQuantumProvider::provision_mldsa_key`] which take explicit seeds.
///
...
impl Default for RustCryptoPqProvider {
    fn default() -> Self {
        fn noop_rng(_buf: &mut [u8]) {
            ...
            // All PQC operations will return NotInitialized until keys are provisioned
            // via the `provision_*` trait methods (which supply explicit seeds and
    ...
    fn try_fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), rand_core::Error> {
        ...
        if !dest.is_empty() {
            ...
            if is_bad {
                ...
                // SAFETY: 0xDEAD is a non-zero constant.
                const RNG_FAIL_CODE: core::num::NonZeroU32 =
                    match core::num::NonZeroU32::new(0xDEAD) {
                        Some(v) => v,
                        None => panic!(
                        "0xDEAD is a non-zero constant; NonZeroU32::new cannot return None here"
                    ),
                    };
...
impl RustCryptoPqProvider {
    ...
    ///
    /// If `seed` is `None`, generates a fresh random seed.
...
impl rand_core::RngCore for SeedRng {
    ...
    fn fill_bytes(&mut self, dest: &mut [u8]) {
        // If seed is exhausted, this is a fatal programming error — silently
        // continuing with degraded randomness would compromise key material.
        // The RngCore trait contract explicitly permits panicking here.
        self.try_fill_bytes(dest).expect(
            "SeedRng: seed exhausted — crypto material would be insecure; \
             this is a bug in seed sizing",
        );
    }
    ...
    fn try_fill_bytes(&mut self, dest: &mut [u8]) -> Result<(), rand_core::Error> {
        ...
        if dest.len() > remaining {
            // F-08: use `new_unchecked` with a compile-time assertion instead
            // of a `match` that unconditionally contains an unreachable `panic!`
            // branch.  The `panic!` branch in a `const` context is evaluated
            // at compile time only if taken; since 0xBEEF != 0 it is never
            // taken.  However, the explicit assertion + `new_unchecked` pattern
            // is cleaner, Clippy-clean, and documents the invariant explicitly.
            const _: () = assert!(0xBEEF != 0, "SEED_EXHAUSTED_CODE must be non-zero");
            // SAFETY: 0xBEEF is a non-zero constant verified by the assert above.
            #[allow(unsafe_code)]
            const SEED_EXHAUSTED_CODE: core::num::NonZeroU32 =
                unsafe { core::num::NonZeroU32::new_unchecked(0xBEEF) };
