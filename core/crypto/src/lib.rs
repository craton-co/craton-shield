...
impl NonceCounter {
    ...
    /// Return the current counter value so callers can persist it to
    /// secure storage before shutdown or periodically during operation.
    ///
    /// On the next boot, pass this value to [`Self::new_persisted`] to
    /// resume with a safe margin that prevents nonce reuse.
    #[deprecated(
        since = "0.7.0",
        note = "use `counter_for_persistence()` instead — identical functionality, clearer name"
    )]
    pub fn last_counter_value(&self) -> u32 {
        self.counter
    }
...
mod tests {
    ...
    #[test]
    #[allow(deprecated)]
    fn nonce_counter_last_counter_value_matches_count() {
        let mut nc = NonceCounter::new([1, 0, 0, 0, 0, 0, 0, 0]).unwrap();
        assert_eq!(nc.last_counter_value(), 0);
        nc.next().unwrap();
        assert_eq!(nc.last_counter_value(), 1);
        nc.next().unwrap();
        nc.next().unwrap();
        assert_eq!(nc.last_counter_value(), 3);
    }
