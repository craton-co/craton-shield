...
fn convert_key(jk: &JsonKey<'_>) -> Result<TufKey, VsError> {
    let key_type = match jk.keytype {
        "ecdsa-sha2-nistp256" | "ecdsa" | "EcdsaP256" => KeyType::EcdsaP256,
...
mod tests {
    ...
    fn parse_tuf_root_minimal() {
        ...
        let (root, metadata) = parse_tuf_root(json.as_bytes()).unwrap();
    ...
    #[allow(deprecated)]
    #[test]
    fn parse_tuf_root_multiple_keys_and_sigs() {
        ...
        let json = format!(
            r#"{{"signatures":[{{"keyid":"{k1_id}","sig":"{sig1}"}},{{"keyid":"{k2_id}","sig":"{sig2}"}},null,null],"signed":{{"version":3,"expires_us":5000000,"threshold":2,"keys":[{{"keyid":"{k1_id}","keytype":"ecdsa","keyval":{{"public":"{pubkey1}"}}}},{{"keyid":"{k2_id}","keytype":"EcdsaP256","keyval":{{"public":"{pubkey2}"}}}},null,null]}}}}"#,
        );
    ...
    #[allow(deprecated)]
    #[test]
    fn parse_tuf_root_invalid_json() {
        let result = parse_tuf_root(b"not json at all");
        assert!(result.is_err());
    }

    #[allow(deprecated)]
    #[test]
    fn parse_tuf_root_bad_hex_key_id() {
        ...
        let result = parse_tuf_root(json.as_bytes());
        assert!(result.is_err());
    }
    ...
    fn parse_tuf_root_with_hash_computes_content_hash() {
        ...
        /// Minimal crypto mock — only sha256 needed.
        struct HashCrypto;
        ...
        // Without hash — content_hash should be zeros
        #[allow(deprecated)]
        let (_, metadata_no_hash) = parse_tuf_root(json.as_bytes()).unwrap();
        assert_eq!(metadata_no_hash.content_hash, [0u8; 32]);

        // With hash — content_hash should be non-zero
    ...
    fn parse_tuf_root_with_per_role_keys() {
        ...
        let json = format!(
            r#"{{"signatures":[{{"keyid":"{root_key_id}","sig":"{sig_hex}"}},null,null,null],"signed":{{"version":1,"expires_us":1000000,"threshold":1,"keys":[{{"keyid":"{root_key_id}","keytype":"ecdsa","keyval":{{"public":"{root_pubkey}"}}}},null,null,null],"targets_keys":[{{"keyid":"{targets_key_id}","keytype":"ecdsa","keyval":{{"public":"{targets_pubkey}"}}}},null,null,null],"targets_threshold":2}}}}"#,
        );

        let (root, _) = parse_tuf_root(json.as_bytes()).unwrap();
