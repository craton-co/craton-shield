...
/// Maximum number of rules a `CanMonitor` can hold.
const MAX_RULES: usize = 256;

/// Capacity of the per-ID statistics hash map (must be a power of two).
...
/// Maximum number of IDs in the allowlist.
const ALLOWLIST_CAPACITY: usize = 512;

/// Maximum number of tracked replay counters (per-ID sequence counters).
    ...
    fn remove(&mut self, id: u32) -> bool {
        ...
        match self.binary_search(id) {
            Ok(pos) => {
                ...
                // Note: we do not re-derive `has_extended` here — it stays
                // true once set.  This is conservative; the constant-time
                // fallback is still correct and remove() is rare.
                true
            }
            Err(_) => false,
        }
...
impl ReplayTracker {
    ...
    /// Hash a CAN ID to an initial slot index (Knuth multiplicative hash).
    #[inline]
    fn id_hash(id: u32) -> usize {
    ...
    fn find_oldest_in_probe(&self, start: usize) -> usize {
        ...
        // Scan a limited window (8 slots) for CLOCK-style approximation.
        let window = 8.min(REPLAY_CAPACITY);
        for i in 0..window {
...
impl StatsMap {
    ...
    fn hash(id: u32) -> usize {
        // Simple multiplicative hash – Knuth's golden-ratio constant.
        let h = (id as u64).wrapping_mul(0x9E37_79B9_7F4A_7C15);
...
fn compute_can_payload_hash(data: &[u8], len: usize) -> vs_types::PayloadHash {
...
impl Default for CanMonitor {
    ...
    ///
    /// Production code must use [`CanMonitor::new`] with a random key.
    fn default() -> Self {
        Self::new([
...
impl CanMonitor {
    ...
    /// # Key validation
    ///
    /// An all-zero key is replaced with a hardcoded non-zero fallback and
    /// the monitor is still created, but replay detection will be weaker.
    /// Use [`CanMonitor::try_new`] for strict validation that rejects
    /// all-zero keys with an error.
    #[allow(clippy::large_stack_arrays)]
    pub fn new(replay_key: [u8; 16]) -> Self {
        // Reject all-zero keys by replacing with a non-zero fallback.
        // This prevents trivially predictable SipHash output while
        // maintaining backward compatibility (no Result return).
        let key = if replay_key.iter().all(|&b| b == 0) {
            // Non-zero fallback — weaker than a random key but still
            // provides basic replay detection.
            [
                0xC4, 0xA7, 0x03, 0x54, 0x1E, 0x1D, 0xCA, 0xFE, 0xDE, 0xAD, 0xBE, 0xEF, 0xF0, 0x0D,
                0xBA, 0xBE,
            ]
        } else {
            replay_key
    ...
    /// Strict variant of [`new`](Self::new) that rejects all-zero keys.
    ///
    /// Returns [`VsError::InvalidInput`] if `replay_key` is all zeros.
    /// Use this in production to ensure a proper random key is supplied.
    #[allow(clippy::large_stack_arrays)]
    ...
    #[deprecated(note = "Use CanMonitor::new(replay_key) instead")]
    #[allow(clippy::large_stack_arrays)]
    pub fn new_with_replay_key(replay_key: [u8; 16]) -> Self {
        Self::new(replay_key)
    }
    ...
    pub fn stats_capacity(&self) -> (usize, usize) {
        let used = self.stats.slots.iter().filter(|s| s.occupied).count();
        (used, STATS_CAPACITY)
    }
    ...
    pub fn process_frame(&mut self, frame: &CanFrame, timestamp_us: u64) -> Option<SecurityAlert> {
        ...
        for i in 0..self.rule_count {
            ...
            if frame.payload_len() as u8 > rule.max_dlc {
                rule_alert = Some(self.make_alert_with_payload(
                    rule.severity,
                    eid,
                    timestamp_us,
            ...
            // 4) Fuzzing (entropy)
            let entropy = Self::shannon_entropy(&frame.data[..frame.payload_len()]);
        ...
        //    detectors.
        let payload_slice = &frame.data[..frame.payload_len()];
    ...
    fn shannon_entropy(data: &[u8]) -> f32 {
        ...
        // Fast path for classic CAN (≤ 8 bytes): use a compact O(n²)
        // uniqueness check instead of allocating a 1 KB frequency table.
        if data.len() <= 8 {
    ...
    /// Shannon entropy for very short payloads (≤ 8 bytes) using O(n²)
    /// counting with only 8 stack bytes instead of 1 KB.
    fn shannon_entropy_small(data: &[u8]) -> f32 {
        let n = data.len();
        if n == 0 {
            return 0.0;
        }
        // Count distinct values and their frequencies using O(n²) scan.
        // For n ≤ 8 this is 28 comparisons at most.
        let mut vals = [0u8; 8];
...
mod tests {
    ...
    fn payload_hash_is_computed() {
        ...
        // Hash should be non-zero since we now compute a FNV-based fingerprint
    ...
    fn replay_tracker_evicts_oldest_when_full() {
        ...
        // Fill the tracker with REPLAY_CAPACITY distinct IDs.
        for i in 0..REPLAY_CAPACITY as u32 {
            tracker.check(i, &[0x01; 4]);
        }

        // Insert one more — should evict the oldest (id=0) and succeed.
        let evicted_id = 0u32;
        let new_id = REPLAY_CAPACITY as u32;
        tracker.check(new_id, &[0xAA; 4]);

        // The new ID should be tracked (second identical payload does not trigger yet).
        assert!(!tracker.check(new_id, &[0xAA; 4]));

        // The evicted ID should be re-inserted fresh (not found from before).
        // It takes a fresh 3 repeats to trigger.
        assert!(!tracker.check(evicted_id, &[0x01; 4])); // 1st
        assert!(!tracker.check(evicted_id, &[0x01; 4])); // 2nd
        assert!(tracker.check(evicted_id, &[0x01; 4])); // 3rd → triggers
    }
    ...
    // -- new_with_replay_key tests -------------------------------------------

    #[test]
    fn new_with_replay_key_creates_functional_monitor() {
        let key = [0xAA; 16];
        #[allow(deprecated)]
        let mut mon = CanMonitor::new_with_replay_key(key);
        let rule = CanRule {
            id: 0,
            id_mask: 0xFFFF_FFFF,
            id_filter: 0x100,
        ...
        assert!(mon.add_rule(rule).is_ok());
    }
