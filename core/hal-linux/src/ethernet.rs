...
use vs_hal::{BatchReceive, EthernetPhy, RawEthFrame, MAX_ETH_FRAME_LEN};
...
/// TTL for the cached `link_speed_mbps` ioctl result (microseconds).
const LINK_SPEED_CACHE_TTL_US: u64 = 1_000_000;

/// Retry a syscall expression while it returns `< 0` and `errno == EINTR`.
///
/// The syscall is restarted unconditionally on `EINTR` (Linux's automatic
/// restart semantics do not apply to non-blocking sockets). All other
/// errors (including `EAGAIN`) propagate as the negative return value so
/// callers can examine `errno` themselves.
...
///
/// Layout matches the kernel `struct ifreq` for ioctl(2): the first
/// `IFNAMSIZ` (= 16) bytes are the `ifr_ifrn.ifrn_name` array, and the
/// remaining union slot holds `ifr_data` (a pointer). Because `IFNAMSIZ`
/// is exactly 16 bytes there is no inter-field padding on either x86_64
/// or aarch64 — the pointer immediately follows the name array at offset
/// 16, matching what the kernel expects for `SIOCETHTOOL`. The struct is
/// `#[repr(C)]` to lock this layout in.
#[repr(C)]
struct EthtoolIfreq {
...
pub struct LinuxEthernetPhy {
    ...
    ///
    /// Avoids issuing a `SIOCETHTOOL` ioctl on every call; link speed
    /// changes rarely in practice.
    cached_speed: core::cell::Cell<Option<(u64, u32)>>,
...
const ETH_MAX_BATCH: usize = 16;

/// Scratch storage for `recvmmsg(2)` reused across batches.
///
/// Pre-zeroed at construction time and reset on each call rather than
/// zero-initialised per syscall. Stays out of public API.
struct BatchScratch {
    iovs: [libc::iovec; ETH_MAX_BATCH],
    hdrs: [libc::mmsghdr; ETH_MAX_BATCH],
}

impl BatchScratch {
    /// Build a new pre-zeroed scratch instance.
    fn new() -> Self {
        // SAFETY: `iovec` and `mmsghdr` are `#[repr(C)]` with only integer
        // and pointer fields. The zero pattern is a valid initialised value
        // (null pointers + zero lengths) that we overwrite before each
        // `recvmmsg` call. Mirrors the previous `mem::zeroed()`-per-call
        // pattern but pays the cost once.
        unsafe {
            Self {
...
impl LinuxEthernetPhy {
    ...
    fn open(interface: &str, protocol: u16) -> Result<Self, VsError> {
        ...
        // Cache interface name. `libc::c_char` is `i8` on x86_64 but `u8`
        // on aarch64; using `0 as libc::c_char` works on both.
        let mut ifname = [0 as libc::c_char; libc::IFNAMSIZ];
    ...
    fn timestamp_us() -> u64 {
        ...
        // CLOCK_MONOTONIC_RAW should never produce a negative tv_sec in
        // practice, but guard against it explicitly to avoid a lossy cast.
    ...
    fn ethtool_speed(&self) -> Option<u32> {
        let mut ecmd: EthtoolCmd = unsafe { core::mem::zeroed() };
        ...
        let ret = retry_on_eintr!(unsafe { libc::ioctl(self.fd, SIOCETHTOOL, &raw mut ifr) });
        if ret < 0 {
            return None;
        }
...
impl EthernetPhy for LinuxEthernetPhy {
    ...
    fn transmit(&mut self, data: &[u8]) -> Result<(), VsError> {
        ...
        if data.len() < MIN_ETH_FRAME_LEN {
            ...
            let n = retry_on_eintr!(unsafe {
                libc::write(self.fd, padded.as_ptr().cast(), MIN_ETH_FRAME_LEN)
            });
            if n < 0 {
                return Err(errno_to_vserror());
            }
        ...
        let n = retry_on_eintr!(unsafe { libc::write(self.fd, data.as_ptr().cast(), data.len()) });
        if n < 0 {
            return Err(errno_to_vserror());
        }
        Ok(())
    ...
    fn link_speed_mbps(&self) -> u32 {
        // Cached for 1s — the ethtool ioctl is non-trivial and the link
        // speed changes very rarely. Cache stored in a Cell because the
        // trait method takes `&self`.
        let now = Self::timestamp_us();
        if let Some((stamp, mbps)) = self.cached_speed.get() {
            if now.saturating_sub(stamp) < LINK_SPEED_CACHE_TTL_US {
                return mbps;
            }
        }
        // Query the actual link speed via ethtool ioctl.
        // Fall back to 1 Gbps if the query fails (e.g. virtual interfaces).
        let mbps = self.ethtool_speed().unwrap_or(1000);
        self.cached_speed.set(Some((now, mbps)));
        mbps
    }
...
impl BatchReceive for LinuxEthernetPhy {
    type Frame = RawEthFrame;

    /// Drain up to `count` Ethernet frames using `recvmmsg(2)`.
    ///
    ...
    fn recv_batch(&mut self, frames: &mut [RawEthFrame], count: usize) -> Result<usize, VsError> {
        ...
        // Reuse the cached scratch arrays across calls instead of paying
        // for a `mem::zeroed()` per-batch. We overwrite all fields we read
        // (`iov_base`, `iov_len`, `msg_iov`, `msg_iovlen`, and `msg_len`
        // from the kernel), so leftover zeros from prior calls are
        // harmless.
        let scratch = &mut self.batch_scratch;

        for i in 0..max {
            scratch.iovs[i].iov_base = frames[i].data.as_mut_ptr().cast();
            scratch.iovs[i].iov_len = MAX_ETH_FRAME_LEN;
            scratch.hdrs[i].msg_hdr.msg_iov = &raw mut scratch.iovs[i];
            scratch.hdrs[i].msg_hdr.msg_iovlen = 1;
            // Reset any stale length from a prior call.
