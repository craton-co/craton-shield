...
//! ## Error mapping
//!
//! Linux `errno` values are translated to [`vs_types::VsError`] by
//! `errno_to_vserror`. Note that `vs_types::VsError` has no
//! `PermissionDenied` variant, so `EACCES`/`EPERM` (typically missing
//! `CAP_NET_RAW`) are mapped to [`vs_types::VsError::BusError`] — this is
//! intentionally lossy and documented at the mapping site.
