...
/// sufficient capacity.  The randomness is supplied internally by the PQ
/// provider's RNG.
///
/// `slot`   — ML-KEM key slot (must have been provisioned with
///            `vs_pq_provision_mlkem_key`).
/// `ct_out` — output: 1088 bytes for the ML-KEM-768 ciphertext.
/// `ss_out` — output: 32 bytes for the shared secret.
#[cfg(feature = "pqc")]
#[no_mangle]
pub unsafe extern "C" fn vs_pq_mlkem_encapsulate(
    ...
    ffi_guard(AssertUnwindSafe(|| {
        ...
        // SAFETY: caller guarantees ct_out/ss_out point to enough writable bytes.
...
/// `ss_out` — output: 32 bytes for the recovered shared secret.
#[cfg(feature = "pqc")]
#[no_mangle]
...
/// `msg_len` — message length in bytes. Must be in `[1, 65535]`. The 64 KiB
///             upper bound prevents stack overflow from large messages in
///             the ML-DSA internal hashing and matches the maximum message
///             size supported by the underlying `ml-dsa` crate.
/// `sig_out` — output: 3309 bytes for the ML-DSA-65 signature.
#[cfg(feature = "pqc")]
#[no_mangle]
...
mod production {
    ...
    pub fn register_provider(provider: Box<dyn CryptoProvider + Send>) {
        if let Ok(mut slot) = PENDING_PROVIDER.lock() {
            *slot = Some(provider);
        }
    }
