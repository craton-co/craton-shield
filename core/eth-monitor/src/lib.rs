...
/// Tracked `DoIP` session keyed by source MAC address.
#[derive(Debug, Clone, Copy)]
struct DoIpSession {
    src_mac: [u8; 6],
...
pub struct EthMonitorConfig {
    ...
    /// Maximum allowed SOME/IP message length field.
...
fn compute_payload_hash(payload: &[u8], keys: &[(u64, u64); 4]) -> [u8; 32] {
    ...
    // Run SipHash-2-4 with four different keys to fill 32 bytes.
...
impl EthMonitor {
    ...
    fn learn_arp(&mut self, ip: [u8; 4], mac: [u8; 6]) -> bool {
        ...
        // Open-addressing insert: probe from the hash slot.
        let start = self.arp_slot_for_ip(&ip);
        for i in 0..MAX_ARP_ENTRIES {
            let idx = (start + i) & (MAX_ARP_ENTRIES - 1);
        ...
        // Table full — evict at the hash slot (unpredictable for attacker).
    ...
    ///
    /// We identify SOME/IP traffic by the well-known UDP port encoded as
    /// `EtherType` 0x8100 (not actually — SOME/IP runs over IP/UDP). In
    /// practice, the caller sets the ethertype to a sentinel or we simply
    /// attempt to parse if the ethertype indicates IPv4 and the payload
    /// contains SOME/IP. For simplicity in this crate, we attempt parsing
    /// unconditionally (non-SOME/IP payloads will fail parsing and be
    /// ignored).
    ...
    fn is_service_allowed(&self, pkt: &EthPacket<'_>, service_id: u16) -> bool {
        ...
        // Cap probe chain at MAX_PROBE_STEPS to bound worst-case latency
        // under adversarial hash collision attacks.
        const MAX_PROBE_STEPS: usize = 16;
        let h = allow_list_hash(&pkt.src_mac, &pkt.dst_mac, service_id);
        let steps = if ALLOW_HASH_CAPACITY < MAX_PROBE_STEPS {
            ALLOW_HASH_CAPACITY
        } else {
            MAX_PROBE_STEPS
        };
    ...
    fn check_doip(&mut self, pkt: &EthPacket<'_>, ts_us: u64) -> Option<SecurityAlert> {
        ...
        match hdr.payload_type {
            ...
            DOIP_ROUTING_ACTIVATION_RESPONSE => {
                ...
                // comes from the expected responder. This prevents an attacker
                // from forging a response to grant Active state to an
                // arbitrary MAC.
                let current = self.doip_get_state(pkt.dst_mac);
                if current == DoIpSessionState::RoutingActivated
                    && self.doip_check_responder(pkt.dst_mac, pkt.src_mac)
                {
                    self.doip_set_state(pkt.dst_mac, DoIpSessionState::Active, ts_us, pkt.src_mac);
                }
                None
            }
            ...
            | DOIP_DIAGNOSTIC_NEGATIVE_ACK => {
                let state = self.doip_get_state(pkt.src_mac);
                match state {
    ...
    fn doip_get_state(&self, mac: [u8; 6]) -> DoIpSessionState {
    ...
    /// Check whether the `responder_mac` matches the expected responder for
    /// the session identified by `session_mac`. Returns `false` if no session
    /// exists or the responder does not match.
    fn doip_check_responder(&self, session_mac: [u8; 6], responder_mac: [u8; 6]) -> bool {
        let mut found = false;
        for session in &self.doip_sessions {
            if vs_types::constant_time_eq(&session.src_mac, &session_mac)
                && session.state != DoIpSessionState::Idle
            {
                found |= vs_types::constant_time_eq(&session.expected_responder, &responder_mac);
            }
        }
        found
    }

    /// Set the state of a `DoIP` session. Returns `true` if an existing
    /// session was evicted to make room for the new one.
    fn doip_set_state(
        &mut self,
        mac: [u8; 6],
        state: DoIpSessionState,
        ...
        for entry in entries.iter().take(count) {
            ...
            match entry.entry_type {
                SdEntryType::OfferService => {
                    ...
                    if self.allow_list_len > 0 && !self.is_mac_in_allow_list(pkt.src_mac) {
                        return Some(self.make_alert(
    ...
    fn sd_track_service(
        ...
        let _ = victim_order; // suppress unused warning
...
fn is_ipv6_extension_header(next_header: u8) -> bool {
    ...
    // Note: ESP (50) is NOT walked because its payload is encrypted.
}
...
mod tests {
    ...
    fn ipv6_with_auth_header_extension() {
        // IPv6 (40) + AH (16, Hdr Ext Len=1 -> (1+1)*8=16) + TCP (20) = 76
        let mut pkt = [0u8; 76];
        pkt[0] = 0x60;
        pkt[5] = 36; // payload_length = 16 + 20
