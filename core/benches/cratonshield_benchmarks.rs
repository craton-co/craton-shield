...
//! Craton Shield core hot-path benchmarks.
//!
//! Each benchmark has an associated **performance budget** that CI watches for
//! regression. Budgets are looser than the v0.7.0 measured means (a ~10-15 %
//! headroom) to absorb runner-to-runner variance on shared GitHub hosts; the
//! intent is to catch *real* regressions, not noise.
//!
//! | Benchmark                                | v0.7.0 mean | CI budget |
//! |------------------------------------------|-------------|-----------|
//! | `can_monitor::process_frame`             |    ~265 ns  |   300 ns  |
//! | `eth_monitor::inspect_packet`            |     ~28 ns  |    40 ns  |
//! | `eth_monitor::inspect_packet_rejected`   |    ~158 ns  |   200 ns  |
//! | `firewall::evaluate/128_rules_first`     |      ~9 ns  |    15 ns  |
//! | `firewall::evaluate/128_rules_last`      |    ~166 ns  |   200 ns  |
//! | `firewall::evaluate/64_port_rules_last`  |    ~252 ns  |   300 ns  |
//! | `policy_engine::evaluate (64 rules)`     |    ~199 ns  |   250 ns  |
//! | `event_logger::append (HMAC-chained)`    |    ~108 ns  |   130 ns  |
//! | `runtime::tick`                          |     ~73 ns  |   100 ns  |
//! | `runtime::submit_can_frame`              |    ~265 ns  |   300 ns  |
//!
//! Source numbers: `core/docs/performance-results.md`. CI policy and review
//! workflow: `PERFORMANCE.md` at the workspace root.
//!
//! `Throughput::Elements(1)` is set on per-frame / per-packet groups so that
//! criterion also reports a frames-per-second figure, which matches how the
//! automotive timing-budget table in `performance-results.md` is framed.
...
fn bench_can_process_frame(c: &mut Criterion) {
    ...
    // Budget: 300 ns / frame (target: 265 ns, 10 us CAN gateway budget).
    let mut group = c.benchmark_group("can_monitor::process_frame");
    group.throughput(Throughput::Elements(1));
    group.bench_function("5_rules", |b| {
        b.iter(|| {
...
fn bench_eth_inspect_packet(c: &mut Criterion) {
    ...
    // Budget: 40 ns / packet (target: 28 ns, hash-indexed allow-list O(1)).
    let mut group = c.benchmark_group("eth_monitor::inspect_packet");
    group.throughput(Throughput::Elements(1));
    group.bench_function("someip_allowed", |b| {
        b.iter(|| {
            ts += 100_000;
...
fn bench_eth_inspect_packet_rejected(c: &mut Criterion) {
    ...
    // Budget: 200 ns / packet (target: 158 ns, worst-case allowlist miss).
    let mut group = c.benchmark_group("eth_monitor::inspect_packet_rejected");
    group.throughput(Throughput::Elements(1));
    group.bench_function("someip_rejected", |b| {
        b.iter(|| {
            ts += 100_000;
            black_box(mon.inspect_packet(black_box(&pkt), black_box(ts)))
        });
...
fn bench_firewall_evaluate(c: &mut Criterion) {
    // Budgets (per evaluate call against the noted rule layout):
    //   128_rules_first_match  ->  15 ns  (target:   9 ns)
    //   128_rules_last_match   -> 200 ns  (target: 166 ns)
    //   64_port_rules_last     -> 300 ns  (target: 252 ns)
...
fn bench_policy_evaluate(c: &mut Criterion) {
    ...
    // Budget: 250 ns / evaluate (target: 199 ns, 64 rules, full-scan miss).
    let mut group = c.benchmark_group("policy_engine::evaluate");
    group.throughput(Throughput::Elements(1));
    group.bench_function("64_rules_miss", |b| {
        b.iter(|| {
...
fn bench_event_logger_append(c: &mut Criterion) {
    ...
    // Budget: 130 ns / append (target: 108 ns, HMAC-chained, cached prev).
    let mut group = c.benchmark_group("event_logger::append");
    group.throughput(Throughput::Elements(1));
    group.bench_function("hmac_chained", |b| {
        b.iter(|| {
            ts += 1_000;
...
fn bench_runtime_tick(c: &mut Criterion) {
    ...
    // Budget: 100 ns / tick (target: 73 ns, idle, 1 ms tick budget).
...
fn bench_runtime_submit_can_frame(c: &mut Criterion) {
    ...
    // Budget: 300 ns / frame (target: 265 ns, full pipeline through IDS+tick).
    let mut group = c.benchmark_group("runtime::submit_can_frame");
    group.throughput(Throughput::Elements(1));
