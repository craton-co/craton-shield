...
/// Size of the snapshot HMAC input prefix.
///
/// Layout (all little-endian, no padding):
///
/// ```text
/// [region_count        : u64 (8B)]
/// [measurement_counter : u64 (8B)]
/// [epoch               : u64 (8B)]
/// [check_interval_ticks: u64 (8B)]
/// [auth_key_id_present : u8  (1B)]
/// [auth_key_id.0       : u32 (4B, LE)]
/// [authenticated       : u8  (1B)]
/// ```
///
/// Followed by per-region data folded in via SHA-256 chaining (see
/// [`snapshot_content_hash`]).
///
/// Binding `auth_key_id_present`, `auth_key_id.0`, and `authenticated`
/// into the digest prevents key-substitution attacks on persisted
/// snapshots: an attacker cannot swap `auth_key_id` for a key they
/// control and re-HMAC the snapshot under that key.
const SNAPSHOT_HMAC_PREFIX_SIZE: usize = 8 + 8 + 8 + 8 + 1 + 4 + 1;
...
pub struct RegionInfo {
    ...
    /// `true` once a tamper event has been detected for this region.
    ///
    /// While latched, the per-region tamper callback is suppressed —
    /// detection still happens (the verification still returns
    /// [`IntegrityStatus::Tampered`]) but the callback fires only once
    /// per latch cycle to avoid alert storms.
    ///
    /// Cleared by [`IntegrityMonitor::acknowledge_tamper`] or by
    /// [`IntegrityMonitor::unregister_region`].
    pub tamper_latched: bool,
}
...
struct IntegrityRegion {
    ...
    /// Set on first tamper detection for this region. While set, the
    /// tamper callback is suppressed (preventing an alert storm when
    /// the same tampered region is re-verified). Cleared by
    /// [`IntegrityMonitor::acknowledge_tamper`] or by
    /// [`IntegrityMonitor::unregister_region`].
    tamper_latched: bool,
}
...
impl IntegrityRegion {
    fn to_info(&self) -> RegionInfo {
        RegionInfo {
            ...
            tamper_latched: self.tamper_latched,
        }
    }
}
...
impl<C: CryptoProvider> IntegrityMonitor<C> {
    ...
    pub fn register_region(
        ...
        self.regions[slot] = IntegrityRegion {
            ...
            active: true,
            last_verified_epoch: 0,
            // Fresh registration starts unlatched so the first detected
            // tamper after this point fires the callback.
            tamper_latched: false,
        };
    ...
    /// baseline cannot be recovered from a memory dump. The region's
    /// tamper latch (see
    /// [`acknowledge_tamper`](Self::acknowledge_tamper)) is also
    /// cleared so that a future re-registration starts with a fresh
    /// (unlatched) state.
    pub fn unregister_region(&mut self, id: u32) -> Result<(), VsError> {
        ...
        self.regions[idx].tamper_latched = false;
        Ok(())
    }

    /// Acknowledge a latched tamper event for a region.
    ///
    /// After a tamper is detected, the region's per-region
    /// `tamper_latched` flag is set and the tamper callback is
    /// suppressed for subsequent detections on the same region. This
    /// API clears that latch so that the next detected tamper event
    /// will once again fire the callback.
    ///
    /// The expected baseline hash is **not** modified — the region
    /// will continue to report
    /// [`IntegrityStatus::Tampered`] on verification until the
    /// underlying memory matches its baseline again (or a legitimate
    /// [`update_baseline`](Self::update_baseline) is performed).
    ///
    /// # Errors
    ///
    /// - [`VsError::NotFound`] — no active region with `region_id`.
    pub fn acknowledge_tamper(&mut self, region_id: u32) -> Result<(), VsError> {
        let idx = self.find_region_index(region_id).ok_or(VsError::NotFound)?;
        self.regions[idx].tamper_latched = false;
        Ok(())
    }
    ...
    /// # Security
    ///
    /// **Prefer [`from_snapshot_with_expected_key`] whenever the caller
    /// knows which `KeyId` should be authenticating the snapshot.**
    ///
    /// This function verifies the HMAC under the `KeyId` recorded *in the
    /// snapshot itself*. The `auth_key_id` field is bound into the HMAC
    /// (see `snapshot_content_hash`), so an attacker who only has the
    /// stored snapshot bytes cannot substitute a different key. However,
    /// an attacker who can both (a) modify the snapshot bytes and (b)
    /// also produce HMAC tags under some other key (e.g. a debug,
    /// test, or provisioning key still loaded in the crypto provider)
    /// could replace `auth_key_id` and re-HMAC. Pinning the expected key
    /// at the call site eliminates this risk.
    ///
    /// Use this variant only when the expected key is unavailable —
    /// for example during early provisioning before the production
    /// auth key is known.
    ///
    /// [`from_snapshot_with_expected_key`]:
    ///     IntegrityMonitor::from_snapshot_with_expected_key
    pub fn from_snapshot(snapshot: &MonitorSnapshot, crypto: C) -> Result<Self, VsError> {
        Self::from_snapshot_inner(snapshot, crypto, None)
    }

    /// Restore a monitor from a snapshot, requiring its HMAC to be
    /// authenticated under `expected_auth_key_id`.
    ///
    /// This is the **preferred** restore entry point. It defends against
    /// key-substitution attacks: before verifying the HMAC, the
    /// `auth_key_id` recorded in the snapshot is compared (constant-time
    /// over a 32-bit value via `==`) against the caller-supplied
    /// `expected_auth_key_id`. A mismatch returns
    /// [`VsError::AuthenticationFailure`] without performing the HMAC
    /// verification.
    ///
    /// Returns [`VsError::AuthenticationFailure`] if either:
    ///
    /// * the snapshot's `auth_key_id` differs from `expected_auth_key_id`,
    /// * the snapshot's `auth_key_id_present` flag is `false` (i.e. the
    ///   snapshot was taken without authentication and so cannot be
    ///   pinned to a key), or
    /// * the HMAC verification under `expected_auth_key_id` fails.
    ///
    /// The tamper callback is **not** persisted — the caller must
    /// re-register it after restoring.
    pub fn from_snapshot_with_expected_key(
        snapshot: &MonitorSnapshot,
        expected_auth_key_id: KeyId,
        crypto: C,
    ) -> Result<Self, VsError> {
        Self::from_snapshot_inner(snapshot, crypto, Some(expected_auth_key_id))
    }

    fn from_snapshot_inner(
        snapshot: &MonitorSnapshot,
        crypto: C,
        expected_auth_key_id: Option<KeyId>,
    ) -> Result<Self, VsError> {
        // Defense-in-depth: when the caller pins an expected key, reject
        // the snapshot if either it claims to be unauthenticated or its
        // recorded `auth_key_id` does not match — *before* touching the
        // HMAC. This stops key-substitution even in the (out-of-scope)
        // scenario where an attacker can produce HMAC tags under some
        // other key currently loaded in the crypto provider.
        if let Some(expected) = expected_auth_key_id {
            if !snapshot.auth_key_id_present {
                return Err(VsError::AuthenticationFailure);
            }
            if snapshot.auth_key_id.0 != expected.0 {
                return Err(VsError::AuthenticationFailure);
            }
        }
        ...
        if snapshot.auth_key_id_present {
            ...
            // Always verify under the key recorded in the snapshot. When
            // the caller pinned `expected_auth_key_id` we have already
            // confirmed it matches, so this is equivalent to verifying
    ...
    /// Fire the tamper callback for `regions[idx]` *unless* the region's
    /// per-region tamper latch is already set.
    ///
    /// On the first detection after registration (or after the latch
    /// was cleared by [`acknowledge_tamper`](Self::acknowledge_tamper)
    /// or [`unregister_region`](Self::unregister_region)), the callback
    /// fires and the latch is set. While latched, subsequent detections
    /// still classify the region as
    /// [`IntegrityStatus::Tampered`] but do not re-invoke the callback,
    /// preventing alert storms when a tampered region is verified
    /// repeatedly.
    fn fire_tamper(&mut self, idx: usize) {
        if self.regions[idx].tamper_latched {
            return;
        }
        self.regions[idx].tamper_latched = true;
        if let Some(cb) = self.tamper_callback {
            let region = &self.regions[idx];
            cb(region.id, region.base_addr, AlertSeverity::Critical);
        }
    }
}
...
/// # Layout
///
/// The digest binds every authentication-relevant field — see
/// [`SNAPSHOT_HMAC_PREFIX_SIZE`] for the exact byte layout. Crucially,
/// `auth_key_id_present`, `auth_key_id.0`, and `authenticated` are
/// included so that an attacker who modifies a persisted snapshot
/// cannot substitute a different `auth_key_id` and re-HMAC under that
/// (attacker-chosen) key. Per-region fields (`id`, `base_addr`,
/// `length`, `expected_hash`, `active`, `last_verified_epoch`) are
/// then folded in via SHA-256 chaining.
///
/// Maintainers: if you add a new field to [`MonitorSnapshot`] that
/// affects authentication state, baseline trust, or replay semantics,
/// you MUST extend this digest to cover it, bump
/// [`SNAPSHOT_HMAC_PREFIX_SIZE`] accordingly, and update the layout
/// documented there.
fn snapshot_content_hash(
    ...
    // Bind authentication identity into the digest — prevents an
    // attacker from substituting `auth_key_id` with a key they control
    // and recomputing the HMAC under that key.
    buf[32] = u8::from(snap.auth_key_id_present);
    buf[33..37].copy_from_slice(&snap.auth_key_id.0.to_le_bytes());
    buf[37] = u8::from(snap.authenticated);
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Regression: per-region tamper latch
    // -----------------------------------------------------------------------

    static TAMPER_FIRE_COUNT: core::sync::atomic::AtomicU32 = core::sync::atomic::AtomicU32::new(0);

    fn counting_tamper_callback(_region_id: u32, _base_addr: usize, _sev: AlertSeverity) {
        TAMPER_FIRE_COUNT.fetch_add(1, Ordering::SeqCst);
    }

    /// The tamper callback must fire **once** when a region is first
    /// detected as tampered. Repeated detections (until acknowledged)
    /// must NOT re-invoke the callback — the per-region latch
    /// suppresses alert storms.
    #[test]
    fn tamper_callback_only_fires_once_until_acknowledged() {
        TAMPER_FIRE_COUNT.store(0, Ordering::SeqCst);

        let mut monitor = IntegrityMonitor::new(TestCrypto::new());
        monitor.set_tamper_callback(counting_tamper_callback);

        let data = b"baseline!";
        monitor.register_region(42, 0x4000, data).expect("register");

        // First detection — callback fires, latch is set.
        let r = monitor
            .verify_region(42, 0x4000, b"TAMPERED1")
            .expect("verify");
        assert_eq!(r.status, IntegrityStatus::Tampered);
        assert_eq!(TAMPER_FIRE_COUNT.load(Ordering::SeqCst), 1);

        let info = monitor.get_region(42).expect("info");
        assert!(info.tamper_latched, "latch must be set after first tamper");

        // Subsequent detections — still classified as Tampered, but
        // callback is suppressed by the latch.
        for _ in 0..5 {
            let r = monitor
                .verify_region(42, 0x4000, b"TAMPERED!")
                .expect("verify");
            assert_eq!(r.status, IntegrityStatus::Tampered);
        }
        assert_eq!(
            TAMPER_FIRE_COUNT.load(Ordering::SeqCst),
            1,
            "callback must only fire once until acknowledged"
        );

        // Same suppression via the verify_all path.
        let mut results = empty_results();
        monitor
            .verify_all(|_, _, _| Some(b"TAMPERED!".as_slice()), &mut results)
            .expect("verify_all");
        assert_eq!(
            TAMPER_FIRE_COUNT.load(Ordering::SeqCst),
            1,
            "verify_all must also honour the latch"
        );

        // And via verify_all_fast.
        let mut failure = IntegrityResult {
            region_id: 0,
            status: IntegrityStatus::Ok,
        };
        monitor
            .verify_all_fast(|_, _, _| Some(b"TAMPERED!".as_slice()), &mut failure)
            .expect("verify_all_fast");
        assert_eq!(
            TAMPER_FIRE_COUNT.load(Ordering::SeqCst),
            1,
            "verify_all_fast must also honour the latch"
        );
    }

    /// After [`acknowledge_tamper`], the latch is cleared and the next
    /// detected tamper event must fire the callback again.
    #[test]
    fn acknowledge_tamper_clears_latch_and_callback_can_fire_again() {
        TAMPER_FIRE_COUNT.store(0, Ordering::SeqCst);
        ...
        let data = b"baseline!";
        monitor.register_region(7, 0x7000, data).expect("register");

        // First detection: latch set, callback fires.
        monitor
            .verify_region(7, 0x7000, b"TAMPERED!")
            .expect("verify");
        assert_eq!(TAMPER_FIRE_COUNT.load(Ordering::SeqCst), 1);
        assert!(monitor.get_region(7).unwrap().tamper_latched);

        // Second detection while latched: suppressed.
        monitor
            .verify_region(7, 0x7000, b"TAMPERED!")
            .expect("verify");
        assert_eq!(TAMPER_FIRE_COUNT.load(Ordering::SeqCst), 1);

        // Acknowledge: latch cleared.
        monitor.acknowledge_tamper(7).expect("acknowledge");
        assert!(!monitor.get_region(7).unwrap().tamper_latched);

        // Next detection: latch re-sets, callback fires again.
        monitor
            .verify_region(7, 0x7000, b"TAMPERED!")
            .expect("verify");
        assert_eq!(TAMPER_FIRE_COUNT.load(Ordering::SeqCst), 2);
        assert!(monitor.get_region(7).unwrap().tamper_latched);

        // acknowledge_tamper on an unknown id returns NotFound.
        assert_eq!(
            monitor.acknowledge_tamper(999),
            Err(VsError::NotFound),
            "acknowledge_tamper must reject unknown ids"
        );

        // unregister_region also clears the latch.
        monitor.unregister_region(7).expect("unregister");
        monitor
            .register_region(7, 0x7000, data)
            .expect("re-register");
        assert!(
            !monitor.get_region(7).unwrap().tamper_latched,
            "re-registered region must start unlatched"
        );
    }
    ...
    // -----------------------------------------------------------------------
    // Regression: auth_key_id is bound into the snapshot HMAC
    // (prevents key-substitution attacks)
    // -----------------------------------------------------------------------

    /// Same content, different `auth_key_id` => different HMAC.
    ///
    /// If `auth_key_id` were NOT bound into the digest, the same input
    /// would be HMAC'd, so different keys producing different tags would
    /// only be guaranteed by HMAC's key-binding. Binding the id into the
    /// digest provides defense-in-depth and makes the property visible
    /// in the message itself.
    #[test]
    fn snapshot_hmac_differs_when_auth_key_id_differs() {
        // Monitor A — key id 1.
        let mut a = IntegrityMonitor::new(TestCrypto::new());
        a.register_region(7, 0x1000, b"identical").unwrap();
        a.set_auth_key(KeyId(1));
        let snap_a = a.snapshot().unwrap();

        // Monitor B — same content, different key id.
        let mut b = IntegrityMonitor::new(TestCrypto::new());
        b.register_region(7, 0x1000, b"identical").unwrap();
        b.set_auth_key(KeyId(2));
        let snap_b = b.snapshot().unwrap();

        assert_ne!(
            snap_a.hmac, snap_b.hmac,
            "snapshots that differ only in auth_key_id must have different HMACs"
        );

        // And the digest input itself must differ.
        let crypto = TestCrypto::new();
        let dig_a = snapshot_content_hash(&crypto, &snap_a).unwrap();
        let dig_b = snapshot_content_hash(&crypto, &snap_b).unwrap();
        assert_ne!(
            dig_a, dig_b,
            "snapshot_content_hash must bind auth_key_id into its digest"
        );
    }

    /// Attacker substitutes `auth_key_id` and re-HMACs under their own
    /// key. Without `from_snapshot_with_expected_key`, this is the
    /// scenario that motivates the new constructor: verification under
    /// the (attacker-controlled) `snapshot.auth_key_id` would succeed
    /// because the attacker computed the tag themselves.
    ///
    /// `from_snapshot_with_expected_key` MUST reject this by checking
    /// the id before the HMAC.
    #[test]
    fn snapshot_key_substitution_rejected_with_expected_key() {
        // Legitimate monitor uses key id 100.
        const LEGIT_KEY: KeyId = KeyId(100);
        const ATTACKER_KEY: KeyId = KeyId(0xDEAD_BEEF);

        let mut monitor = IntegrityMonitor::new(TestCrypto::new());
        monitor.register_region(1, 0x1000, b"baseline").unwrap();
        monitor.set_auth_key(LEGIT_KEY);
        let legit_snap = monitor.snapshot().unwrap();

        // Sanity: legit snapshot verifies fine under the expected key.
        assert!(
            IntegrityMonitor::from_snapshot_with_expected_key(
                &legit_snap,
                LEGIT_KEY,
            ...
            .is_ok(),
            "legitimate snapshot must verify under expected key"
        );

        // Attacker takes the persisted snapshot, swaps the key id to a
        // key they control, and re-HMACs the (modified) snapshot under
        // their key.
        let mut forged = legit_snap;
        forged.auth_key_id = ATTACKER_KEY;
        let crypto = TestCrypto::new();
        let digest = snapshot_content_hash(&crypto, &forged).unwrap();
        crypto
            .hmac_sha256(ATTACKER_KEY, &digest, &mut forged.hmac)
            .unwrap();

        // The forged snapshot's HMAC is *self-consistent*:
        // `from_snapshot` (the legacy API) would accept it.
        assert!(
            IntegrityMonitor::from_snapshot(&forged, TestCrypto::new()).is_ok(),
            "self-consistent forgery is accepted by legacy from_snapshot \
             (this is exactly the attack from_snapshot_with_expected_key prevents)"
        );

        // The hardened API MUST reject it because the key id does not
        // match the caller's expectation.
        let result = IntegrityMonitor::from_snapshot_with_expected_key(
            &forged,
            LEGIT_KEY,
            TestCrypto::new(),
        )
        ...
        assert!(
            matches!(result, Err(VsError::AuthenticationFailure)),
            "key-substitution forgery must be rejected by from_snapshot_with_expected_key, \
             got {result:?}"
        );
    }

    /// `from_snapshot_with_expected_key` must reject unauthenticated
    /// snapshots: there is no key to pin.
    #[test]
    fn from_snapshot_with_expected_key_rejects_unauthenticated() {
        let mut monitor = IntegrityMonitor::new(TestCrypto::new());
        monitor.register_region(1, 0x1000, b"data").unwrap();
        // No set_auth_key call — snapshot will be unauthenticated.
        let snap = monitor.snapshot().unwrap();
        assert!(!snap.auth_key_id_present);

        let result =
            IntegrityMonitor::from_snapshot_with_expected_key(&snap, KeyId(1), TestCrypto::new());
        assert!(
            matches!(result, Err(VsError::AuthenticationFailure)),
            "unauthenticated snapshot must be rejected when expected key is supplied"
        );
    }
}
