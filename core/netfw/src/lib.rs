...
impl FirewallRule {
    ...
    /// Check whether L3/L4 fields match.  If no L3/L4 fields are set,
    /// this is a pure L2 rule and always matches.
    fn matches_l3l4(&self, ip: Option<&ParsedL3L4>) -> bool {
        let has_l3_fields = self.src_ip.is_some()
            || self.dst_ip.is_some()
...
impl Firewall {
    ...
    pub fn rule_capacity(&self) -> (usize, usize) {
        let active = self.rules[..self.rule_count]
            .iter()
    ...
    pub fn evaluate(&mut self, pkt: &EthPacket<'_>, ts_us: u64) -> Verdict {
        ...
        let parsed = parse_ip(pkt.ethertype, pkt.payload).map(|(ip, offset)| {
        ...
        for i in 0..self.rule_count {
            ...
            if rule.matches_l2(pkt) && rule.matches_l3l4(parsed_ref) {
                best = Some(i);
                break;
            }
        }
    ...
    fn insert_slot(&mut self, slot: RuleSlot) -> Result<(), VsError> {
        ...
        self.rules[self.rule_count] = slot;
        self.rule_count = self.rule_count.saturating_add(1);
        self.sort_rules();
        Ok(())
    }

    /// Sort active rules by ascending priority (lower number = higher priority)
    /// using insertion sort. This is O(n^2) worst-case but rules are added
    /// infrequently and the array is nearly-sorted, making insertion sort
    /// near-O(n) in practice.
    ...
    fn rate_limit_check(&mut self, rule_id: u32, rate: u32, now_us: u64) -> bool {
        ...
        // Table is full — evict the least-recently-used entry (oldest
        // `last_update_us`) so that stale rate limiters don't permanently
        // occupy slots and cause new rule IDs to be dropped.
        let mut lru_idx: usize = 0;
        let mut lru_ts: u64 = u64::MAX;
        for i in 0..MAX_RATE_LIMITERS {
            if self.rate_limiters[i].active && self.rate_limiters[i].last_update_us < lru_ts {
                lru_ts = self.rate_limiters[i].last_update_us;
                lru_idx = i;
            }
        }
