...
impl FileStorageProvider {
    ...
    pub fn entry_count(&self) -> Result<usize, VsError> {
        ...
        for entry in entries {
            ...
            // Skip atomic-write temporaries.  Real value files are named
            // by hex-encoded keys, which never contain `.tmp`; the temp
            // names produced by `atomic_write_file` contain the literal
            // substring `.tmp` (see [`is_tmp_name`]).
            if is_tmp_name(name_str) {
                continue;
            }
            count += 1;
        }
    ...
    /// Read the value for `key` directly into `buf` without an intermediate
    /// heap allocation.
    ///
    /// Returns the number of bytes written. Unlike [`StorageProvider::read`]
    /// (which performs a `std::fs::read` and therefore heap-allocates a
    /// `Vec<u8>` sized to the file before copying into the caller's buffer),
    /// this method opens the file and reads directly into `buf`.
    ///
    /// `key` is the raw 32-byte key. The buffer must be large enough to hold
    /// the stored value, else [`VsError::InvalidInput`] is returned.
    ///
    /// # Errors
    ///
    /// * [`VsError::NotFound`]         – key does not exist.
    /// * [`VsError::InvalidInput`]     – `buf` is shorter than the stored value.
    /// * [`VsError::IntegrityFailure`] – stored value exceeds [`MAX_VALUE_LEN`].
    /// * [`VsError::StorageError`]     – underlying I/O error.
    pub fn read_into(&self, key: &[u8; 32], buf: &mut [u8]) -> Result<usize, VsError> {
        ...
        let meta = file.metadata().map_err(|_| VsError::StorageError)?;
        ...
        while total < len {
            match file.read(&mut buf[total..len]) {
                Ok(0) => return Err(VsError::StorageError),
                Ok(n) => total += n,
                Err(ref e) if e.kind() == std::io::ErrorKind::Interrupted => continue,
                Err(_) => return Err(VsError::StorageError),
            }
        }
        Ok(total)
    }
...
/// Return `true` if `name` is an atomic-write temporary file produced by
/// [`atomic_write_file`].
///
/// Temporary files use the pattern `<hex>.tmpXXXXXXXXXXXXXXXX` — the original
/// hex-encoded key, a literal `.tmp` separator, then 16 hex digits derived
/// from a counter mixed with the PID. Real value files are bare hex with no
/// extension, so the presence of `.tmp` is sufficient to distinguish them.
#[inline]
...
impl StorageProvider for FileStorageProvider {
    /// Heap-allocating convenience read.
    ///
    /// This implementation calls [`std::fs::read`], which allocates a
    /// `Vec<u8>` sized to the on-disk value before copying into the caller's
    /// `buf`. For zero-allocation hot paths, use
    /// [`FileStorageProvider::read_into`] instead, which reads directly into
    /// `buf` without an intermediate allocation.
    ...
    fn for_each_key(&self, f: &mut dyn FnMut(&[u8]) -> bool) -> Result<(), VsError> {
        ...
        for entry in entries {
            ...
            // Skip subdirectories and other non-regular entries: only files
            // produced by `atomic_write_file` are valid value containers.
            // A subdirectory whose name happens to be valid hex must not be
            // surfaced as a key.
            ...
            let name = entry.file_name();
            // F-09: non-UTF-8 filenames are not produced by this provider;
            // skip them rather than aborting the iteration so a single
            // foreign file cannot make the entire store unenumerable.
            let name_str = match name.to_str() {
                Some(s) => s,
                None => continue,
            };

            // Skip atomic-write temporaries (see [`is_tmp_name`]).
            if is_tmp_name(name_str) {
                continue;
            }
            ...
            // Skip filenames that do not decode as hex.  Such files are not
            // produced by this provider; aborting iteration over them would
            // let a single stray file (e.g. an editor swap file) hide all
            // real keys from callers.
            let mut key_buf = [0u8; MAX_KEY_LEN];
            let key_len = match hex_decode(name_str, &mut key_buf) {
                Ok(n) => n,
                Err(_) => continue,
            };
...
fn atomic_write_file(path: &std::path::Path, data: &[u8]) -> Result<(), VsError> {
    ...
    // Mix counter with process ID for less predictable temp file names.
    // The (PID, monotonically-increasing seq) pair is unique within the
    // lifetime of the host kernel's PID-reuse window, which is sufficient
    // for uniqueness on a single base_dir.  A previous version mixed in
    // `SystemTime::now()`, but that incurred a syscall on every write
    // without strengthening uniqueness — the counter already guarantees it.
    let mixed = seq
    ...
    // Sync parent directory for rename durability (POSIX).  An EIO on the
    // parent fsync is a real durability issue — the rename may not be
    // persisted on power loss — so propagate it instead of swallowing.
    // `File::open(parent)` may legitimately fail on some filesystems that
    // do not allow opening a directory for fsync (rare on Linux); in that
    // case we accept the weaker guarantee rather than fail the write.
    #[cfg(unix)]
    if let Some(parent) = path.parent() {
        if let Ok(dir) = std::fs::File::open(parent) {
            dir.sync_all().map_err(|_| VsError::StorageError)?;
        }
    }
...
#[cfg(windows)]
// TODO(perf): replace icacls subprocess with SetNamedSecurityInfoW via the
// `windows-sys` crate.  The current child-process spawn adds a DoS surface
// (an attacker who can stall the icacls binary stalls every write) and
// costs ~10ms per atomic write on typical Windows hosts.  A direct
// `SetNamedSecurityInfoW` + `BuildExplicitAccessWithNameW` call is both
// faster and removes the dependency on the icacls binary being on PATH.
