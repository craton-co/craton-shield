...
/// Serialize an audit entry into a fixed-size byte buffer for hashing.
/// Encodes position-dependent data to prevent reorder attacks.
...
pub struct KeyManager<C: CryptoProvider> {
    ...
    /// Rolling SHA-256 checksum over all live audit entries.
...
impl<C: CryptoProvider> KeyManager<C> {
    ...
    pub fn new(crypto: C) -> Self {
        Self {
            keys: core::array::from_fn(|_| KeyEntry::default()),
    ...
    /// Verify integrity of the audit log by recomputing the checksum
    /// using the crypto provider's SHA-256.
    pub fn verify_audit_integrity(&self) -> Result<bool, VsError> {
        ...
        for entry in self.audit.iter().flatten() {
            let entry_bytes = audit_entry_to_bytes(entry);
            // Hash(running || entry_bytes) to create a chain
            let mut to_hash = [0u8; 32 + 25];
            to_hash[..32].copy_from_slice(&running);
            to_hash[32..].copy_from_slice(&entry_bytes);
            self.crypto.sha256(&to_hash, &mut running)?;
        }
        Ok(vs_types::constant_time_eq_32(
            &running,
            &self.audit_checksum,
        ))
    }
    ...
    fn append_audit(
        ...
        self.audit[self.audit_head] = Some(entry);
        self.audit_head = (self.audit_head + 1) % AUDIT_CAPACITY;
        self.audit_count = self.audit_count.saturating_add(1);

        if is_overflow {
            // An old entry was evicted; the chained hash must be recomputed
            // from scratch because we cannot "subtract" a hash-chain element.
            self.recompute_audit_checksum();
        } else {
            // No eviction — extend the chain incrementally with a single
            // SHA-256 call instead of recomputing all 256 entries.
            self.extend_audit_checksum(&entry);
        }
        Ok(())
    }
    ...
    /// Extend the audit checksum with a single new entry (O(1) SHA-256).
    ///
    /// Retries transient crypto failures up to `CHECKSUM_RETRY_LIMIT` times
    /// before marking the checksum invalid, preventing a single glitch from
    /// permanently disabling audit integrity.
    fn extend_audit_checksum(&mut self, entry: &AuditEntry) {
        let entry_bytes = audit_entry_to_bytes(entry);
        let mut to_hash = [0u8; 32 + 25];
        to_hash[..32].copy_from_slice(&self.audit_checksum);
        to_hash[32..].copy_from_slice(&entry_bytes);
        for _ in 0..Self::CHECKSUM_RETRY_LIMIT {
            if self
                .crypto
                .sha256(&to_hash, &mut self.audit_checksum)
                .is_ok()
            {
                // Success — checksum remains valid.
                return;
            }
        }
        // All retries exhausted — mark invalid so callers can detect the
        // degradation and take corrective action.
        self.checksum_valid = false;
        self.audit_checksum = [0u8; 32];
    }

    /// Recompute the audit checksum from scratch over all live entries.
    /// Called only on ring buffer overflow when an entry is evicted.
    fn recompute_audit_checksum(&mut self) {
        let mut running = [0u8; 32];
        for entry in self.audit.iter().flatten() {
            let entry_bytes = audit_entry_to_bytes(entry);
            let mut to_hash = [0u8; 32 + 25];
            to_hash[..32].copy_from_slice(&running);
            to_hash[32..].copy_from_slice(&entry_bytes);
            if self.crypto.sha256(&to_hash, &mut running).is_err() {
                self.checksum_valid = false;
                self.audit_checksum = [0u8; 32];
                return;
            }
        }
        self.audit_checksum = running;
    }
    ...
    pub fn keym_finalize(&mut self) {
        for entry in &mut self.keys {
            ...
            entry.metadata.key_id = KeyId::new(0);
