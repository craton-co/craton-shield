...
impl ModbusMonitor {
    ...
    pub fn add_address_rule(&mut self, rule: AddressRule) -> Result<(), VsError> {
        ...
        self.rules[idx] = StoredRule { rule };
        self.rule_count += 1;
        Ok(())
    }
    ...
    pub fn parse_tcp(bytes: &[u8], timestamp_us: u64) -> Result<ModbusTcpFrame, Verdict> {
        ...
        let mut frame = ModbusTcpFrame::with_pdu(
            transaction_id,
            protocol_id,
            unit_id,
            fc,
        ...
        //
        // FC 0x16 (Mask Write Register) is decoded specially: its PDU
        // layout is [fc, addr_hi, addr_lo, and_hi, and_lo, or_hi, or_lo]
        // and the request targets a single register, so we record the
        // address and a quantity of 1.
        if pdu_len_usize >= 5 && raw_fc & MODBUS_EXCEPTION_BIT == 0 {
            ...
            match raw_fc {
                ...
                0x16 => {
                    // Mask Write Register: single register at `address`.
                    frame.start_address = address;
                    frame.quantity = 1;
                }
                _ => {}
    ...
    pub fn inspect_rtu(&mut self, frame: &ModbusRtuFrame) -> (Verdict, ModbusInspectResult) {
        ...
        if frame.crc_provided {
            ...
            buf[1..=pdu_len].copy_from_slice(&frame.pdu()[..pdu_len]);
            let computed = crc16_modbus_fast(&buf[..=pdu_len]);
    ...
    fn inspect_pdu(
        ...
        //
        // Exception responses (function code with the high bit set) are
        // *normal* Modbus protocol behaviour: a slave indicating it
        // could not service a request (illegal address, illegal data
        // value, slave busy, etc.). They are not denials of service or
        // policy violations on the wire. We therefore deliberately
        // leave `result.allowed = true` and emit a low-severity alert
        // so that the frame still reaches the SIEM pipeline for
        // correlation without being dropped or treated as malicious.
        ...
        for stored in &self.rules[..self.rule_count as usize] {
            let r = &stored.rule;
...
/// Precomputed CRC-16/Modbus byte-table.
///
/// Each entry `i` is the CRC of a single byte `i` started from
/// `0`, computed with polynomial `0xA001`. Built at compile time so the
/// table sits in `.rodata` (512 bytes) and incurs no runtime
/// initialization cost.
pub const CRC16_MODBUS_TABLE: [u16; 256] = crc16_modbus_table();
...
/// Compute the CRC-16-IBM/ANSI used by Modbus RTU, byte-table variant.
///
/// Polynomial `0xA001` (reversed `0x8005`), init `0xFFFF`, no final XOR.
/// Uses the precomputed [`CRC16_MODBUS_TABLE`] for one table lookup per
/// input byte instead of eight bit-shifts. This is the variant used by
/// [`ModbusMonitor::inspect_rtu`].
pub fn crc16_modbus_fast(data: &[u8]) -> u16 {
...
/// Compute the CRC-16-IBM/ANSI used by Modbus RTU, bit-by-bit variant.
///
/// Polynomial `0xA001` (reversed `0x8005`), init `0xFFFF`, no final XOR.
/// Kept as a spec-reference / test oracle; production code should call
/// [`crc16_modbus_fast`] (or its alias [`crc16_modbus`]) instead.
pub fn crc16_modbus_slow(data: &[u8]) -> u16 {
    let mut crc: u16 = 0xFFFF;
    for &byte in data {
        crc ^= u16::from(byte);
        for _ in 0..8 {
...
///
/// Alias for [`crc16_modbus_fast`] — the table-driven variant. Retained
/// for source compatibility with earlier revisions.
#[inline]
pub fn crc16_modbus(data: &[u8]) -> u16 {
    crc16_modbus_fast(data)
}
