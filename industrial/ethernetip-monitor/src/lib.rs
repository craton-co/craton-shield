...
impl EtherNetIpMonitor {
    ...
    pub fn inspect(&mut self, frame: &EtherNetIpFrame) -> EtherNetIpInspectResult {
        ...
        if frame.command == REGISTER_SESSION {
            ...
            // For all non-lifecycle commands, refresh the session's idle
            // timer so active sessions are not prematurely expired.
            self.touch_session(frame.session_handle, frame.timestamp_us);
        }

        let is_lifecycle = frame.command == REGISTER_SESSION || frame.command == UNREGISTER_SESSION;
        if !is_lifecycle && self.strict_mode && !self.session_known(frame.session_handle) {
            result.allowed = false;
            result.push_alert_with_code(
        ...
        if frame.command == UNREGISTER_SESSION {
            ...
            for i in 0..self.rule_count as usize {
                if self.rules[i].active
                    && (self.rules[i].allowed_command == 0
                        || self.rules[i].allowed_command == frame.command)
                {
                    matched_rule = Some(i);
                    break;
                }
    ...
    fn register_session(&mut self, handle: u32, ts_us: u64) {
        // If already registered, refresh last_activity_us and return.
        for s in &mut self.sessions {
        ...
        // Find empty slot or evict the session with the oldest created_us.
        ...
        for (i, s) in self.sessions.iter().enumerate() {
            ...
            } else if s.created_us < oldest_ts {
                oldest_ts = s.created_us;
                oldest_idx = i;
            }
        }
    ...
    fn session_known(&self, handle: u32) -> bool {
        self.sessions
            .iter()
            .any(|s| s.active && s.session_handle == handle)
    }

    /// Update `last_activity_us` for a known session so idle timeout resets.
    fn touch_session(&mut self, handle: u32, ts_us: u64) {
        for s in &mut self.sessions {
            if s.active && s.session_handle == handle {
                s.last_activity_us = ts_us;
                return;
...
mod tests {
    ...
    fn session_lru_eviction_when_table_full() {
        ...
        // All registered handles should be known.
        let f = EtherNetIpFrame {
            command: 0x0070,
            session_handle: 1000,
            timestamp_us: 2000,
        ...
        // Register one more → should evict the oldest (handle 1000).
        let reg = EtherNetIpFrame {
            command: REGISTER_SESSION,
            session_handle: 9999,
            timestamp_us: 10000,
        ...
        // The oldest handle (1000) should have been evicted — unknown in strict mode.
        let f = EtherNetIpFrame {
            command: 0x0070,
            session_handle: 1000,
            timestamp_us: 11000,
