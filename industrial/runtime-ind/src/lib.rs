...
// Re-export protocol monitor crates so consumers don't need to depend on
...
pub struct IndustrialHealth {
    /// Core platform health snapshot.
    pub core: PlatformHealth,
    /// Modbus (TCP and RTU) subsystem status.
    pub modbus: SubsystemStatus,
    /// OPC UA subsystem status.
    pub opcua: SubsystemStatus,
    /// PROFINET subsystem status.
    pub profinet: SubsystemStatus,
    /// EtherNet/IP subsystem status.
    pub ethernetip: SubsystemStatus,
    /// DNP3 subsystem status.
    pub dnp3: SubsystemStatus,
    /// `BACnet` subsystem status.
    pub bacnet: SubsystemStatus,
    /// S7comm subsystem status.
    pub s7comm: SubsystemStatus,
    /// IEC 60870-5-104 subsystem status.
    pub iec60870: SubsystemStatus,
    /// IEC 61850 (MMS and GOOSE) subsystem status.
...
// ---------------------------------------------------------------------------
// Shared macros for protocol submit methods
// ---------------------------------------------------------------------------

/// Route alerts and toggle the matching subsystem to `Degraded`.
///
/// Invoked from each `submit_*` method after the protocol monitor returns
/// `result`. Consolidates the alert-routing + degrade-bookkeeping that
/// would otherwise be repeated identically across 10 protocols.
macro_rules! handle_alerts {
    (
        $self:ident,
        $result:ident,
        $payload_hash:expr,
...
/// Emit a `submit_*_zoned` method that wraps the non-zoned submit with a
/// conduit check.
///
/// `from_zone` and `to_zone` are directional: a conduit registered as
/// `(A → B)` does not permit traffic in the `(B → A)` direction. Returns
/// `Err(VsError::PolicyViolation)` if no conduit allows `$proto`.
macro_rules! impl_zoned_submit {
    (
        $zoned_name:ident,
        $inner_name:ident,
        $frame_ty:ty,
        ...
        #[doc = concat!("Submit a ", $proto_label, " frame with conduit enforcement.")]
        ///
        /// `from_zone` and `to_zone` are **directional**: a conduit
        /// registered as `(A → B)` does not permit traffic in the
        /// `(B → A)` direction. Callers must pass the zones in the order
        /// the traffic flows.
        ///
        /// Returns `Err(VsError::PolicyViolation)` if no conduit allows
        /// the protocol between the given zones.
        pub fn $zoned_name(
            &mut self,
            frame: &$frame_ty,
            from_zone: u8,
            to_zone: u8,
            ts_us: u64,
        ) -> Result<$result_ty, VsError> {
            if !self.check_conduit(from_zone, to_zone, $proto) {
                return Err(VsError::PolicyViolation);
            }
            Ok(self.$inner_name(frame, ts_us))
        }
    };
}
...
impl<C: CryptoProvider + Clone> IndustrialShield<C> {
    ...
    /// Fallible convenience constructor with default crypto.
    ///
    /// Equivalent to `init(*config, C::default())`. Prefer this over
    /// [`Self::new`] when the `CryptoProvider`'s `Default` impl can fail its
    /// self-test or otherwise return an error.
    ...
    /// panic will halt the device. Use [`Self::try_new`] or [`Self::init`]
    /// directly when you need to handle initialisation failures gracefully
    /// (e.g. when the `CryptoProvider` can fail its self-test).
    ...
    /// Submit a CAN frame to the underlying core runtime for inspection.
    pub fn submit_can_frame(&mut self, frame: &CanFrame, ts_us: u64) -> Result<(), VsError> {
        self.core.submit_can_frame(frame, ts_us)
    }

    /// Submit an Ethernet packet to the underlying core runtime for inspection.
    pub fn submit_eth_packet(&mut self, pkt: &EthPacket<'_>, ts_us: u64) -> Result<(), VsError> {
        self.core.submit_eth_packet(pkt, ts_us)
    }

    // -----------------------------------------------------------------------
    // Protocol submit (macro-generated)
    // -----------------------------------------------------------------------
    //
    // The 10 industrial submit methods (Modbus TCP/RTU, OPC UA, PROFINET,
    // EtherNet/IP, DNP3, BACnet, S7comm, IEC 60870-5-104, IEC 61850 MMS,
    // IEC 61850 GOOSE) follow an identical post-inspect routine:
    //   1. compute a payload hash (or `PayloadHash::ZERO` for protocols that
    //      don't retain the raw PDU);
    //   2. route alerts through the core and store them in the recent buffer;
    //   3. flip the subsystem to `Degraded` on High/Critical severity.
    //
    // The `handle_alerts!` macro captures step 2-3 to shrink `.text` and
    // ensure all protocols stay in lockstep. The non-zoned submit methods
    // remain individually written because each protocol has a different
    // payload-hash extraction (Modbus uses `pdu_data`, OPC UA hashes the
    // endpoint URL when present, S7/IEC60870/IEC61850 retain no raw bytes).
    // The zoned wrappers are uniformly generated by `impl_zoned_submit!`.
    ...
    pub fn submit_modbus_tcp(&mut self, frame: &ModbusTcpFrame, ts_us: u64) -> ModbusInspectResult {
        ...
        let payload_hash = Self::compute_hash(self.core.crypto(), frame.pdu());
        handle_alerts!(
            self,
            result,
            payload_hash,
            modbus_status,
            modbus_degraded_since_us,
            ts_us
        );
        result
    }
    ...
    pub fn submit_opcua_message(&mut self, msg: &OpcUaMessage, ts_us: u64) -> OpcUaInspectResult {
        ...
        let payload_hash = if msg.endpoint_len() > 0 {
            Self::compute_hash(self.core.crypto(), msg.endpoint())
        } else {
            PayloadHash::ZERO
        };
        handle_alerts!(
    ...
    pub fn submit_profinet_frame(
        ...
        let payload_hash = Self::compute_hash(self.core.crypto(), frame.payload());
        handle_alerts!(
            self,
            result,
            payload_hash,
            profinet_status,
            profinet_degraded_since_us,
            ts_us
        );
        result
    }
    ...
    ///
    /// S7comm frames carry header-level metadata (function code, PDU type,
    /// SZL ID, rate-limit state) rather than a raw payload buffer on
    /// `S7commFrame`, so alerts attach `PayloadHash::ZERO`.
    ...
    ///
    /// IEC 60870-5-104 frames are decoded into structured fields (TypeID,
    /// COT, ASDU address, I-frame sequence) and do not retain the raw ASDU
    /// bytes, so alerts attach `PayloadHash::ZERO`.
    ...
    ///
    /// IEC 61850 MMS frames are parsed into service/object identifiers before
    /// reaching this layer; the raw MMS PDU is not retained on `MmsFrame`,
    /// so alerts attach `PayloadHash::ZERO`.
    ...
    ///
    /// IEC 61850 GOOSE frames are decoded into publisher/StNum/SqNum metadata
    /// used for replay detection; the underlying APDU bytes are not retained
    /// on `GooseFrame`, so alerts attach `PayloadHash::ZERO`.
    ...
    impl_zoned_submit!(
        submit_modbus_tcp_zoned,
        submit_modbus_tcp,
        ModbusTcpFrame,
        ModbusInspectResult,
        vs_types_ind::PROTO_MODBUS_TCP,
        "Modbus TCP"
    );
    impl_zoned_submit!(
        submit_modbus_rtu_zoned,
        submit_modbus_rtu,
        ModbusRtuFrame,
        ModbusInspectResult,
        vs_types_ind::PROTO_MODBUS_RTU,
        "Modbus RTU"
    );
    impl_zoned_submit!(
        submit_opcua_message_zoned,
        submit_opcua_message,
        OpcUaMessage,
        OpcUaInspectResult,
        vs_types_ind::PROTO_OPCUA,
        "OPC UA"
    );
    impl_zoned_submit!(
        submit_profinet_frame_zoned,
        submit_profinet_frame,
        ProfinetFrame,
        ProfinetInspectResult,
        vs_types_ind::PROTO_PROFINET,
        "PROFINET"
    );
    impl_zoned_submit!(
        submit_ethernetip_frame_zoned,
        submit_ethernetip_frame,
        EtherNetIpFrame,
        EtherNetIpInspectResult,
        vs_types_ind::PROTO_ETHERNETIP,
        "EtherNet/IP"
    );
    impl_zoned_submit!(
        submit_dnp3_frame_zoned,
        submit_dnp3_frame,
        Dnp3Frame,
        Dnp3InspectResult,
        vs_types_ind::PROTO_DNP3,
        "DNP3"
    );
    impl_zoned_submit!(
        submit_bacnet_frame_zoned,
        submit_bacnet_frame,
        BacnetFrame,
        BacnetInspectResult,
        vs_types_ind::PROTO_BACNET,
        "`BACnet`"
    );
    impl_zoned_submit!(
        submit_s7comm_frame_zoned,
        submit_s7comm_frame,
        S7commFrame,
        S7commInspectResult,
        vs_types_ind::PROTO_S7COMM,
        "S7comm"
    );
    impl_zoned_submit!(
        submit_iec60870_frame_zoned,
        submit_iec60870_frame,
        Iec60870Frame,
        Iec60870InspectResult,
        vs_types_ind::PROTO_IEC60870,
        "IEC 60870-5-104"
    );
    impl_zoned_submit!(
        submit_iec61850_mms_zoned,
        submit_iec61850_mms,
        MmsFrame,
        Iec61850MmsInspectResult,
        vs_types_ind::PROTO_IEC61850,
        "IEC 61850 MMS"
    );
    impl_zoned_submit!(
        submit_iec61850_goose_zoned,
        submit_iec61850_goose,
        GooseFrame,
        Iec61850GooseInspectResult,
        vs_types_ind::PROTO_IEC61850,
        "IEC 61850 GOOSE"
    ...
    /// has been reached.
    ///
    /// Configuration-time only: the duplicate check performs a linear scan
    /// over the conduit table. Do not call from hot paths.
    pub fn add_conduit(
        &mut self,
        from_zone: u8,
        to_zone: u8,
    ...
    ///
    /// Called on every zoned submit. Linear scan over the conduit table is
    /// fine while `MAX_CONDUITS` is small (≤ 255).
    // TODO(perf 0.8): consider a `(from, to) -> allowed_protocols` 2D lookup
    // table or hash to make this O(1) when the conduit table is large.
...
mod tests {
    ...
    fn read_holding_tcp() -> ModbusTcpFrame {
        let mut pdu = [0u8; 5];
        pdu[0] = 0x03;
        pdu[1..3].copy_from_slice(&0u16.to_be_bytes());
        pdu[3..5].copy_from_slice(&1u16.to_be_bytes());
        ModbusTcpFrame::with_pdu(
            1,
            0,
            1,
            vs_types_ind::ModbusFunctionCode::from_u8(0x03),
            0x03,
            0,
            1,
            &pdu,
    ...
    #[test]
    fn industrial_try_new_constructor() {
        let config = PlatformConfig::default();
        let shield: IndustrialShield<TestCrypto> = IndustrialShield::try_new(&config).unwrap();
        assert!(shield.is_initialized());
    }
    ...
    fn industrial_profinet_integration() {
        ...
        f.frame_type = vs_types_ind::ProfinetFrameType::CyclicRT;
        f.frame_id = 0x8000;
        f.timestamp_us = 1000;
    ...
    fn industrial_ethernetip_integration() {
        let mut shield = make_shield();
        // `session_handle == 0` is the reserved sentinel and is rejected on
        // any non-`RegisterSession` command (see ethernetip-monitor); use a
        // non-zero handle so the permissive monitor accepts the frame.
    ...
    fn industrial_profinet_dcp_blocked_routes_alert() {
        let mut shield = make_shield();
        let mut f = ProfinetFrame::default();
        f.frame_type = vs_types_ind::ProfinetFrameType::Dcp;
        f.timestamp_us = 1000;
    ...
    fn health_recovery_opcua() {
        ...
        msg.msg_type = vs_types_ind::OpcUaMessageType::Write;
        msg.security_mode = vs_types_ind::OpcUaSecurityMode::SignAndEncrypt;
        msg.channel_id = 1;
    ...
    fn ethernetip_submit_valid_allowed() {
        let mut shield = make_shield();
        // Non-zero `session_handle` required: the reserved zero sentinel is
        // rejected for any non-`RegisterSession` command (see ethernetip-monitor).
        let f = EtherNetIpFrame {
            session_handle: 1,
            ..Default::default()
        };
