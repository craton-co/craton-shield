...
//!
//! Validates link-layer and pairing traffic against the Bluetooth Core
//! Specification 5.3 (Vol. 3, Part H — SMP; Vol. 6 — Link Layer).
//!
...
/// ATT MTU was renegotiated to a smaller value (possible downgrade).
const ALERT_MTU_DOWNGRADE: u32 = 15;
/// GATT operation hit a handle requiring auth/authz but the link is not
/// authenticated/authorized.
const ALERT_GATT_PERM_VIOLATION: u32 = 16;
/// Pairing method changed in a security-weakening direction
/// (Secure Connections to Legacy, or authenticated to JustWorks).
const ALERT_PAIRING_DOWNGRADE: u32 = 17;
const ALERT_ADV_REPLAY: u32 = 18;

/// Sentinel for "no ATT MTU baseline yet" stored in `PeerState::last_att_mtu`.
const ATT_MTU_NO_BASELINE: u16 = 0;

/// Maximum number of GATT permission policies the monitor can hold.
const MAX_GATT_HANDLE_POLICIES: usize = 32;
...
/// Maximum number of advertisement-digest entries retained for replay detection.
///
/// The ring is shared across all advertisers (not per-MAC) because per-MAC
/// rings would multiply by `MAX_TRACKED_PEERS` and bloat stack usage; the
/// digest already incorporates the peer MAC.
const MAX_ADV_DIGEST_RING: usize = 16;

/// Default advertisement replay window (5 seconds).  Two advertisements with
/// the same `(peer_addr, rssi, conn_handle)` digest seen within this window
/// are considered a replay.
const DEFAULT_ADV_REPLAY_WINDOW_US: u64 = 5_000_000;
...
struct PeerState {
    ...
    /// Last seen negotiated ATT MTU (0 = no baseline established).
    last_att_mtu: u16,
    /// Last seen pairing method (None = no baseline established).
    last_pairing_method: Option<PairingMethod>,
    /// Last seen LE Secure Connections flag (None = no baseline established).
    last_pairing_secure_conn: Option<bool>,
    active: bool,
}
...
impl PeerState {
    const fn empty() -> Self {
        Self {
            ...
            last_att_mtu: ATT_MTU_NO_BASELINE,
            last_pairing_method: None,
            last_pairing_secure_conn: None,
            active: false,
        }
    }
}
...
// ---------------------------------------------------------------------------
// GATT permission policy
// ---------------------------------------------------------------------------

/// Per-handle GATT permission requirements.
#[derive(Debug, Clone, Copy)]
struct GattHandlePolicy {
    handle: u16,
    /// Read/write requires an authenticated link.
    require_auth: bool,
    /// Read/write requires application-level authorization.
    require_authz: bool,
    active: bool,
}

impl GattHandlePolicy {
    const fn empty() -> Self {
        Self {
            handle: 0,
            require_auth: false,
...
/// Advertisement-replay ring buffer entry.
///
/// Stores a 32-bit FNV-1a digest of the advertisement-identifying fields
/// (`peer_addr`, `rssi`, `conn_handle`) and the timestamp at which the
/// matching event was observed.  Used by [`BleMonitor`] to detect identical
/// advertisements seen within a configurable replay window.
#[derive(Debug, Clone, Copy)]
struct AdvDigestEntry {
    digest: u32,
    timestamp_us: u64,
    active: bool,
}
...
pub struct BleMonitor {
    ...
    /// Per-handle GATT permission policies.
    gatt_policies: [GattHandlePolicy; MAX_GATT_HANDLE_POLICIES],
    gatt_policy_count: u8,
    /// Advertisement replay-detection ring buffer.
    adv_digest_ring: [AdvDigestEntry; MAX_ADV_DIGEST_RING],
    adv_digest_write_idx: u8,
    adv_replay_window_us: u64,
...
impl BleMonitor {
    ...
    pub fn new() -> Self {
        Self {
            ...
            gatt_policies: [GattHandlePolicy::empty(); MAX_GATT_HANDLE_POLICIES],
            gatt_policy_count: 0,
            adv_digest_ring: [AdvDigestEntry::empty(); MAX_ADV_DIGEST_RING],
            adv_digest_write_idx: 0,
            adv_replay_window_us: DEFAULT_ADV_REPLAY_WINDOW_US,
            next_alert_id: 1,
            total_inspected: 0,
            total_alerts: 0,
        }
    ...
    /// Register a GATT handle permission policy.
    ///
    /// A subsequent read or write on `handle` will emit
    /// `ALERT_GATT_PERM_VIOLATION` (High) when the event's
    /// `gatt_perm_flags` does not satisfy `require_auth` /
    /// `require_authz`.  Re-registering an existing handle updates the
    /// policy in place.
    pub fn set_gatt_handle_policy(
        &mut self,
        handle: u16,
        require_auth: bool,
        require_authz: bool,
        ...
        for slot in &mut self.gatt_policies {
            if slot.active && slot.handle == handle {
                slot.require_auth = require_auth;
                slot.require_authz = require_authz;
                return Ok(());
            }
        }
        if self.gatt_policy_count as usize >= MAX_GATT_HANDLE_POLICIES {
            return Err(VsError::ResourceExhausted);
        }
        for slot in &mut self.gatt_policies {
            if !slot.active {
                ...
                slot.require_authz = require_authz;
                slot.active = true;
                self.gatt_policy_count = self.gatt_policy_count.saturating_add(1);
                return Ok(());
            }
        }
    ...
    /// Remove all registered GATT handle policies.
    pub fn clear_gatt_handle_policies(&mut self) {
        self.gatt_policies = [GattHandlePolicy::empty(); MAX_GATT_HANDLE_POLICIES];
        self.gatt_policy_count = 0;
    }
    ...
    /// # Detectors
    ///
    /// The monitor evaluates each event against the following detectors,
    /// recording up to four alerts in the returned [`BleInspectResult`]:
    ///
    /// - **Timestamp anomaly** — non-monotonic / out-of-range timestamps
    ///   (alert source 8).
    /// - **Invalid MAC** — zero or broadcast peer addresses (alert source 14).
    /// - **MAC allow/blocklist** — explicit MAC filter actions
    ///   (alert source 1).
    /// - **Connection storm** — sliding-window connection rate
    ///   (alert source 2).
    /// - **RSSI anomaly / relay attack** — out-of-range or large delta
    ///   between consecutive readings (alert source 3).
    /// - **Peer-slot exhaustion** — peer table full (alert source 4).
    /// - **Pairing-failure lockout** — per-peer consecutive pairing failures
    ///   (alert source 5).
    /// - **Global pairing storm** — distributed pairing-failure rate across
    ///   all peers (alert source 6).
    /// - **GATT abuse** — per-peer GATT read/write rate (alert source 7).
    /// - **ATT MTU downgrade** — negotiated MTU shrinks below the previously
    ///   observed baseline (alert source 15).
    /// - **GATT permission violation** — operation on a policy-protected
    ///   handle without the required auth / authz flags (alert source 16).
    /// - **Pairing-method downgrade** — Secure Connections to Legacy, or
    ///   authenticated to JustWorks (alert source 17).
    /// - **Advertisement replay** — duplicate `(peer_addr, rssi, conn_handle)`
    ///   digest within the replay window (alert source 18).
    /// - **Random address flood** — surge of random-class BLE addresses,
    ///   including random-private-non-resolvable addresses detected via the
    ///   IEEE 802 locally-administered bit (alert source 9).
    /// - **Pairing-request flood** — per-peer pairing-request rate
    ///   (alert source 10).
    /// - **Short connection** — disconnect within
    ///   `MIN_CONNECTION_DURATION_US` of connect (alert source 11).
    /// - **Advertisement flood** — adv rate across all peers
    ///   (alert source 12).
    /// - **Unknown BLE event** — unrecognised `BleEventType` variant
    ///   (alert source 13).
    ///
    /// Future schema enhancements (full advertisement-payload digesting,
    /// explicit HCI address-type flag) are tracked in `vs-types-embedded`.
    ...
    pub fn inspect(&mut self, event: &BleEvent) -> BleInspectResult {
        ...
        match event.event_type {
            BleEventType::Connected => {
                ...
                let (rssi_anomaly, mtu_downgrade, slot_exhausted) = {
                    let (peer, old_rssi) =
                    ...
                    if let Some(peer) = peer {
                        ...
                        let mtu_downgrade = update_peer_mtu(peer, event.att_mtu);
                        (rssi_anomaly, mtu_downgrade, false)
                    } else {
                        (false, false, true)
                    }
                };
                ...
                // Random address flood detection.  Use the LA-bit augmented
                // heuristic so random-private-non-resolvable addresses (0b00
                // MSB + locally-administered bit set), which
                // `BleAddressType::classify` cannot distinguish from public
                // addresses without an HCI flag, are still counted.
                if ble_address_is_random(&event.peer_addr) {
                    if event
                        .timestamp_us
            ...
            BleEventType::GattRead | BleEventType::GattWrite => {
                // Lookup the permission policy for this attribute handle
                // (if any) BEFORE borrowing the peer mutably.  Fall back
                // to `conn_handle` if the dedicated `gatt_handle` field is
                // not populated, to preserve compatibility with stacks
                // that only surface the connection handle.
                let policy = event
                    .gatt_handle
                    .or(Some(event.conn_handle))
                    .and_then(|h| self.find_gatt_policy(h));

                let threshold = self.gatt_rate_threshold;
                let (exceeded, mtu_downgrade, slot_exhausted) = {
                    let (peer, _) = self.get_or_create_peer(event.peer_addr, event.timestamp_us);
                    if let Some(peer) = peer {
                        ...
                        // ATT MTU downgrade detection.
                        let mtu_downgrade = update_peer_mtu(peer, event.att_mtu);
                        (exceeded, mtu_downgrade, false)
                    } else {
                        (false, false, true)
                    }
                };
                ...
                if let Some(p) = policy {
                    if check_gatt_perm_violation(p, event.gatt_perm_flags) {
                        result.allowed = false;
                        let aid = self.next_alert_id();
                        result.push_alert(
                            AlertSeverity::High,
                            ALERT_GATT_PERM_VIOLATION,
                            event.timestamp_us,
                            aid,
                        );
                    }
            ...
            BleEventType::AdvertisementReceived => {
                ...
                // Advertisement replay detection: digest the per-event metadata
                // (peer MAC, RSSI, conn handle) and look up the digest in the
                // ring buffer.  A match within `adv_replay_window_us` is a
                // replay — emit an alert but continue processing flood/RSSI
                // bookkeeping above (already done) for completeness.
                if self.detect_adv_replay(event) {
                    let aid = self.next_alert_id();
                    result.push_alert(
                        AlertSeverity::Medium,
                        ALERT_ADV_REPLAY,
            ...
            BleEventType::PairingComplete => {
                let (downgrade, slot_exhausted) = {
                    let (peer, _) = self.get_or_create_peer(event.peer_addr, event.timestamp_us);
                    if let Some(peer) = peer {
                        peer.pairing_failures = 0;
                        peer.pairing_requests = 0;
                        let downgrade = detect_pairing_downgrade(
                            peer.last_pairing_method,
                            peer.last_pairing_secure_conn,
                            event.pairing_method,
                            event.pairing_secure_conn,
                        );
                        // Always record the most recent values we have.
                        if let Some(m) = event.pairing_method {
                            peer.last_pairing_method = Some(m);
                        }
                        if let Some(sc) = event.pairing_secure_conn {
                            peer.last_pairing_secure_conn = Some(sc);
                        }
                        (downgrade, false)
                    } else {
                        (false, true)
            ...
            // Future BleEventType variants (the enum is #[non_exhaustive])
            // are treated as unknown for alerting purposes.
            _ => {
                let aid = self.next_alert_id();
                result.push_alert(
                    AlertSeverity::Medium,
    ...
    fn get_or_create_peer(
        ...
        // --- Pass 2: scan for first free slot; only compute LRU candidates
        // when no free slot exists.  Pass 1 above is constant-time for
        // side-channel reasons; pass 2 runs only on a cache miss and can
        // skip the LRU bookkeeping when a free slot is available.
        ...
        let slot = if free_slot.is_some() {
            free_slot
        } else {
            // No free slot — find an LRU candidate.
    ...
    /// Look up the permission policy for a GATT attribute handle.
    #[inline]
    fn find_gatt_policy(&self, handle: u16) -> Option<GattHandlePolicy> {
        for slot in &self.gatt_policies {
            if slot.active && slot.handle == handle {
                return Some(*slot);
            }
        }
    ...
    /// Record a connection timestamp into the storm-detection ring buffer.
    ///
    /// # Invariants
    ///
    /// `MAX_CONN_TIMESTAMPS` must be `<= 256` so that `conn_write_idx: u8` can
    /// represent every ring index.  `wrapping_add(1)` is correct because the
    /// ring length is a power of two factor of 256 (current value: 32) — when
    /// `conn_write_idx` reaches `MAX_CONN_TIMESTAMPS` we wrap back via the
    /// modulo on the next call's `idx` computation.
    #[inline]
    fn record_connection(&mut self, ts_us: u64) {
        ...
        self.conn_write_idx = (self.conn_write_idx.wrapping_add(1)) % (MAX_CONN_TIMESTAMPS as u8);
        if (self.conn_count as usize) < MAX_CONN_TIMESTAMPS {
            self.conn_count += 1;
        }
    }

    // TODO(perf): maintain a sliding count by evicting expired ring entries
    // at insertion time so this becomes O(1) instead of
    // O(`MAX_CONN_TIMESTAMPS`).  The ring is written in insertion order so
    // the oldest entry is at `conn_write_idx` (when full) — a small refactor
    // can short-circuit by checking that slot's timestamp against `start`.
    ...
    /// Record a global pairing-failure timestamp.  Same `MAX <= 256` invariant
    /// as [`Self::record_connection`].
    #[inline]
    fn record_global_pairing_fail(&mut self, ts_us: u64) {
        ...
        self.global_pairing_fail_write_idx =
            (self.global_pairing_fail_write_idx.wrapping_add(1)) % (MAX_GLOBAL_PAIRING_TS as u8);
        if (self.global_pairing_fail_count as usize) < MAX_GLOBAL_PAIRING_TS {
            self.global_pairing_fail_count += 1;
        }
    }

    // TODO(perf): same sliding-count optimisation opportunity as
    // `detect_conn_storm` above.
    ...
    /// Detect an advertisement replay.
    ///
    /// Computes a 32-bit FNV-1a digest over the advertisement-identifying
    /// fields available on `BleEvent` (`peer_addr`, `rssi`, `conn_handle`),
    /// then scans the ring buffer for a matching, non-expired entry.  On a
    /// match returns `true`; otherwise records the digest into the ring
    /// and returns `false`.
    ///
    /// The ring uses a write-replace policy: when full, the oldest slot is
    /// overwritten.  Expired entries (older than `adv_replay_window_us`) are
    /// treated as if absent and may be overwritten unconditionally.
    fn detect_adv_replay(&mut self, event: &BleEvent) -> bool {
        let digest = adv_event_digest(event);
        let window = self.adv_replay_window_us;
        let now = event.timestamp_us;

        // Pass 1: look for an existing, non-expired match.  Touch every slot
        // (the ring is small) to keep behaviour uniform.
        let mut replay_match = false;
        let mut overwrite_idx: Option<usize> = None;
        let mut oldest_ts = u64::MAX;
        for i in 0..MAX_ADV_DIGEST_RING {
            let entry = &self.adv_digest_ring[i];
            let expired = now.saturating_sub(entry.timestamp_us) > window;
            if entry.active && !expired && entry.digest == digest {
                replay_match = true;
            }
            // Pick the oldest (or first inactive) slot as the overwrite target.
            if !entry.active {
                if overwrite_idx.is_none() {
                    overwrite_idx = Some(i);
                }
            } else if entry.timestamp_us < oldest_ts {
                oldest_ts = entry.timestamp_us;
                if overwrite_idx.map_or(true, |idx| self.adv_digest_ring[idx].active) {
                    overwrite_idx = Some(i);
                }
            }
        }

        // Record the digest into the ring (write-replace).  On a replay match
        // we still refresh the entry's timestamp so a sustained replay keeps
        // raising alerts on each subsequent duplicate within the window.
        let slot = overwrite_idx.unwrap_or_else(|| {
            let s = (self.adv_digest_write_idx as usize) % MAX_ADV_DIGEST_RING;
            self.adv_digest_write_idx =
                ((self.adv_digest_write_idx as usize + 1) % MAX_ADV_DIGEST_RING) as u8;
            s
        });
        self.adv_digest_ring[slot] = AdvDigestEntry {
            digest,
            timestamp_us: now,
            active: true,
        };
...
/// Update a peer's ATT MTU baseline.  Returns `true` if the newly observed
/// MTU is strictly smaller than the previously seen value (downgrade).
///
/// A `None` MTU (event without MTU context) leaves the baseline unchanged.
/// The first non-None MTU sets the baseline and never triggers a downgrade
/// alert.  An equal or larger MTU updates the baseline and does not alert.
#[inline]
fn update_peer_mtu(peer: &mut PeerState, new_mtu: Option<u16>) -> bool {
    let Some(mtu) = new_mtu else {
        return false;
    };
    // BLE ATT MTU is at least 23 octets (Core Spec v5, Vol 3 Part F section
    // 3.2.8).  Zero is the sentinel for "no baseline".  Treat MTUs below
    // the minimum legal value as no-op for baseline-tracking purposes.
    if mtu < 23 {
        return false;
    }
    if peer.last_att_mtu == ATT_MTU_NO_BASELINE {
        peer.last_att_mtu = mtu;
        return false;
    }
    let downgrade = mtu < peer.last_att_mtu;
    // Always update to the latest observed value so subsequent downgrades
    // are measured against the most recent baseline (otherwise an attacker
    // could oscillate the MTU and only the first drop would alert).
    peer.last_att_mtu = mtu;
    downgrade
}

/// Returns `true` when the GATT permission flags fail to satisfy the
/// per-handle policy requirements.
///
/// A missing (`None`) `gatt_perm_flags` field is treated as "no permissions
/// asserted by the stack" and therefore violates any policy that requires
/// auth or authz.
#[inline]
fn check_gatt_perm_violation(policy: GattHandlePolicy, perm_flags: Option<u8>) -> bool {
    if !policy.require_auth && !policy.require_authz {
        return false;
    }
    let flags = perm_flags.unwrap_or(0);
    let peer_authenticated = !policy.require_auth || (flags & GATT_PERM_AUTHENTICATED) != 0;
    let access_authorized = !policy.require_authz || (flags & GATT_PERM_AUTHORIZED) != 0;
    !(peer_authenticated && access_authorized)
}

/// Detect a pairing-method downgrade between the previous and newly
/// observed pairing.
///
/// A downgrade is flagged when:
///   * the previous pairing was LE Secure Connections and the new one is
///     Legacy, OR
///   * the previous method was authenticated (Passkey / Numeric / OOB) and
///     the new one is `JustWorks`.
///
/// The first observation (no prior baseline) never triggers a downgrade.
#[inline]
fn detect_pairing_downgrade(
    prev_method: Option<PairingMethod>,
    prev_secure: Option<bool>,
    new_method: Option<PairingMethod>,
    new_secure: Option<bool>,
) -> bool {
    let secure_downgrade = matches!((prev_secure, new_secure), (Some(true), Some(false)));
    let method_downgrade = match (prev_method, new_method) {
        (Some(prev), Some(new)) => prev.is_authenticated() && !new.is_authenticated(),
        _ => false,
    };
...
/// available on `BleEvent`.
///
/// Covers `peer_addr`, `rssi`, and `conn_handle` — the only advertisement
/// metadata exposed by the current `BleEvent` schema.  The advertisement
/// payload (`adv_data`) is not currently included on the wire-format type;
/// a `vs-types-embedded` change is tracked separately to add it, after
/// which this digest should be extended.
#[inline]
fn adv_event_digest(event: &BleEvent) -> u32 {
    let mut buf = [0u8; 6 + 1 + 2];
    buf[0..6].copy_from_slice(&event.peer_addr);
    buf[6] = event.rssi as u8;
    buf[7..9].copy_from_slice(&event.conn_handle.to_le_bytes());
    vs_types_embedded::fnv1a_hash(&buf)
}

/// Detect whether a BLE address is a random address, augmenting
/// [`BleAddressType::classify`] with detection of random-private-non-resolvable
/// addresses.
///
/// `BleAddressType::classify` only flags `0b11` (random static) and `0b01`
/// (random private resolvable) because the `0b00` MSB pattern is ambiguous
/// between a true public address and a random-private-non-resolvable address
/// without the HCI address-type flag.  `BleEvent` currently does not carry
/// that flag (a `vs-types-embedded` schema change tracked separately), so
/// we apply an IEEE-802 heuristic here:
///
/// - The IEEE 802 "locally administered" bit (bit 1 / mask `0x02`) of the
///   OUI most-significant byte (`addr[5]` in BLE little-endian order) is
///   set on locally-administered MAC addresses.  Genuine public BLE
///   addresses use an OUI assigned to a vendor, which by IEEE convention
///   clears this bit.  When `0b00` MSB AND `addr[5] & 0x02 != 0`, the
///   address is, with very high probability, a random-private-non-resolvable
///   address rather than a public one.
///
/// This conservatively classifies addresses with the LA bit set as random;
/// the small false-positive rate is acceptable for flood detection (only
/// raises a Medium alert), and the alternative — missing the entire random
/// address class — defeats the detector against the most common
/// privacy-rotation strategy.
///
/// # Future work
///
/// When `BleEvent` is extended with an `is_random` HCI flag
/// (`vs-types-embedded` change, tracked separately), prefer
/// [`BleAddressType::classify_with_hci_flag`] for unambiguous classification.
#[inline]
fn ble_address_is_random(addr: &[u8; 6]) -> bool {
    if BleAddressType::classify(addr).is_random() {
        return true;
    }
    // 0b00 MSB + locally-administered bit set → random-private-non-resolvable.
    let msb_pattern = addr[5] >> 6;
    let locally_administered = (addr[5] & 0x02) != 0;
    msb_pattern == 0b00 && locally_administered
}
...
impl MonitorReset for BleMonitor {
    fn reset_state(&mut self) {
        ...
        self.adv_digest_ring = [AdvDigestEntry::empty(); MAX_ADV_DIGEST_RING];
        self.adv_digest_write_idx = 0;
        self.next_alert_id = 1;
        ...
        // gatt_rate_threshold, random_addr_threshold, gatt_policies,
        // gatt_policy_count.
    }
}
...
mod tests {
    ...
    fn make_event(peer: [u8; 6], evt_type: BleEventType, rssi: i8, ts_us: u64) -> BleEvent {
        BleEvent {
            ...
            timestamp_us: ts_us,
            att_mtu: None,
            gatt_perm_flags: None,
            pairing_method: None,
            pairing_secure_conn: None,
            gatt_handle: None,
        }
    }
    ...
    fn alert_source_id_constants_are_nonzero() {
        ...
        assert_ne!(ALERT_MTU_DOWNGRADE, 0);
        assert_ne!(ALERT_GATT_PERM_VIOLATION, 0);
        assert_ne!(ALERT_PAIRING_DOWNGRADE, 0);
    }
    ...
    /// Construct a GATT-read event with an explicit `att_mtu`.
    fn make_gatt_event(peer: [u8; 6], handle: u16, mtu: Option<u16>, ts_us: u64) -> BleEvent {
        BleEvent {
            event_type: BleEventType::GattRead,
            peer_addr: peer,
            rssi: -50,
            conn_handle: 1,
            timestamp_us: ts_us,
            att_mtu: mtu,
            gatt_handle: Some(handle),
            ..BleEvent::default()
        }
    }

    #[test]
    fn mtu_downgrade_detected_on_gatt_event() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000); // avoid GATT abuse alert

        // First MTU sets the baseline; no alert.
        let r1 = mon.inspect(&make_gatt_event(PEER_A, 1, Some(247), 1_000_000));
        let has_dg =
            (0..r1.alert_count as usize).any(|i| r1.alerts[i].source_id == ALERT_MTU_DOWNGRADE);
        assert!(!has_dg, "first MTU observation should not alert");

        // Subsequent smaller MTU triggers downgrade alert.
        let r2 = mon.inspect(&make_gatt_event(PEER_A, 1, Some(23), 2_000_000));
        let has_dg =
            (0..r2.alert_count as usize).any(|i| r2.alerts[i].source_id == ALERT_MTU_DOWNGRADE);
        assert!(has_dg, "smaller MTU should trigger ALERT_MTU_DOWNGRADE");
        let sev = (0..r2.alert_count as usize)
            .find(|&i| r2.alerts[i].source_id == ALERT_MTU_DOWNGRADE)
            .map(|i| r2.alerts[i].severity);
        assert_eq!(sev, Some(AlertSeverity::Medium));
    }

    #[test]
    fn mtu_equal_or_larger_does_not_alert() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000);

        let _ = mon.inspect(&make_gatt_event(PEER_A, 1, Some(100), 1_000_000));
        // Equal MTU – no alert.
        let r = mon.inspect(&make_gatt_event(PEER_A, 1, Some(100), 2_000_000));
        let has_dg =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_MTU_DOWNGRADE);
        assert!(!has_dg);
        // Larger MTU – no alert.
        let r = mon.inspect(&make_gatt_event(PEER_A, 1, Some(247), 3_000_000));
        let has_dg =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_MTU_DOWNGRADE);
        assert!(!has_dg);
    }
    ...
    #[test]
    fn mtu_below_minimum_ignored() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000);

        // MTU below ATT minimum (23) is treated as no-op and never sets
        // a baseline.
        let _ = mon.inspect(&make_gatt_event(PEER_A, 1, Some(10), 1_000_000));
        // No baseline yet, so the next legal MTU also does not alert.
        let r = mon.inspect(&make_gatt_event(PEER_A, 1, Some(247), 2_000_000));
        let has_dg =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_MTU_DOWNGRADE);
        assert!(!has_dg);
    }
    ...
    #[test]
    fn mtu_downgrade_detected_on_connected_event() {
        let mut mon = BleMonitor::new();

        // Initial connection with MTU 247.
        let evt1 = BleEvent {
            event_type: BleEventType::Connected,
            peer_addr: PEER_A,
            rssi: -50,
        ...
        let r = mon.inspect(&evt2);
        let has_dg =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_MTU_DOWNGRADE);
        assert!(has_dg, "MTU downgrade on Connected should alert");
    }

    // -----------------------------------------------------------------------
    // GATT permission violation
    // -----------------------------------------------------------------------

    #[test]
    fn gatt_perm_violation_detected_when_auth_missing() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000);
        mon.set_gatt_handle_policy(0x42, true, false).unwrap();

        // Read with no permission flags set – authentication required but
        // absent.
        let evt = BleEvent {
            event_type: BleEventType::GattRead,
            peer_addr: PEER_A,
            ...
            conn_handle: 1,
            timestamp_us: 1_000_000,
            gatt_handle: Some(0x42),
            gatt_perm_flags: Some(0),
            ..BleEvent::default()
        };
        let r = mon.inspect(&evt);
        assert!(!r.allowed, "perm violation should deny the event");
        let has_viol =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_GATT_PERM_VIOLATION);
        assert!(has_viol);
        let sev = (0..r.alert_count as usize)
            .find(|&i| r.alerts[i].source_id == ALERT_GATT_PERM_VIOLATION)
            .map(|i| r.alerts[i].severity);
        assert_eq!(sev, Some(AlertSeverity::High));
    }

    #[test]
    fn gatt_perm_violation_detected_when_flags_missing() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000);
        mon.set_gatt_handle_policy(0x43, true, true).unwrap();

        // None flags should be treated as no permissions asserted.
        let evt = BleEvent {
            event_type: BleEventType::GattWrite,
            peer_addr: PEER_A,
            rssi: -50,
            conn_handle: 1,
            timestamp_us: 1_000_000,
            gatt_handle: Some(0x43),
            gatt_perm_flags: None,
            ..BleEvent::default()
        };
        let r = mon.inspect(&evt);
    ...
    #[test]
    fn gatt_perm_ok_when_authenticated_and_authorized() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000);
        mon.set_gatt_handle_policy(0x44, true, true).unwrap();
        ...
        let evt = BleEvent {
            ...
            conn_handle: 1,
            timestamp_us: 1_000_000,
            gatt_handle: Some(0x44),
            gatt_perm_flags: Some(GATT_PERM_AUTHENTICATED | GATT_PERM_AUTHORIZED),
            ..BleEvent::default()
        };
        let r = mon.inspect(&evt);
        let has_viol =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_GATT_PERM_VIOLATION);
        assert!(!has_viol, "satisfied policy should not alert");
        assert!(r.allowed);
    }

    #[test]
    fn gatt_perm_unregistered_handle_passes() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000);
        // No policy registered for handle 0x99.
        let evt = BleEvent {
            event_type: BleEventType::GattRead,
            peer_addr: PEER_A,
            rssi: -50,
    ...
    #[test]
    fn gatt_handle_policy_update_in_place() {
        let mut mon = BleMonitor::new();
        // Initial policy – auth required.
        mon.set_gatt_handle_policy(0x55, true, false).unwrap();
        // Update – relax to no auth.
        mon.set_gatt_handle_policy(0x55, false, false).unwrap();
        mon.set_gatt_rate_threshold(10_000);
        ...
        let has_viol =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_GATT_PERM_VIOLATION);
        assert!(!has_viol, "relaxed policy should not alert");
    }

    #[test]
    fn gatt_handle_policy_full_returns_error() {
        let mut mon = BleMonitor::new();
        for i in 0..MAX_GATT_HANDLE_POLICIES as u16 {
            mon.set_gatt_handle_policy(i, true, false).unwrap();
        }
        // One past capacity returns ResourceExhausted.
        let r = mon.set_gatt_handle_policy(0xFFFF, true, false);
        assert!(r.is_err());
    }

    #[test]
    fn clear_gatt_handle_policies_disables_check() {
        let mut mon = BleMonitor::new();
        mon.set_gatt_rate_threshold(10_000);
        mon.set_gatt_handle_policy(0x66, true, false).unwrap();
        mon.clear_gatt_handle_policies();
    ...
    // -----------------------------------------------------------------------
    // Pairing-method downgrade
    // -----------------------------------------------------------------------

    fn make_pair_complete(
        peer: [u8; 6],
        ...
        BleEvent {
            event_type: BleEventType::PairingComplete,
            peer_addr: peer,
            rssi: -50,
            conn_handle: 1,
            timestamp_us: ts_us,
            pairing_method: method,
            pairing_secure_conn: secure,
            ..BleEvent::default()
        }
    }

    #[test]
    fn pairing_downgrade_secure_to_legacy() {
        let mut mon = BleMonitor::new();
        // First pairing: LE Secure Connections, Numeric.
        let _ = mon.inspect(&make_pair_complete(
            PEER_A,
            Some(PairingMethod::Numeric),
            Some(true),
            1_000_000,
        ));
        // Re-pair with Legacy – downgrade.
        let r = mon.inspect(&make_pair_complete(
            PEER_A,
            Some(PairingMethod::Passkey),
            Some(false),
            2_000_000,
        ));
        assert!(!r.allowed);
        let has_dg =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_PAIRING_DOWNGRADE);
        assert!(has_dg);
        let sev = (0..r.alert_count as usize)
            .find(|&i| r.alerts[i].source_id == ALERT_PAIRING_DOWNGRADE)
            .map(|i| r.alerts[i].severity);
        assert_eq!(sev, Some(AlertSeverity::High));
    }

    #[test]
    fn pairing_downgrade_passkey_to_just_works() {
        let mut mon = BleMonitor::new();
        let _ = mon.inspect(&make_pair_complete(
            PEER_A,
            Some(PairingMethod::Passkey),
        ...
        // Re-pair with JustWorks (still secure_conn=true to isolate the
        // method-only downgrade dimension).
        let r = mon.inspect(&make_pair_complete(
            PEER_A,
    ...
    #[test]
    fn pairing_first_observation_no_alert() {
        let mut mon = BleMonitor::new();
        let r = mon.inspect(&make_pair_complete(
            PEER_A,
            Some(PairingMethod::JustWorks),
        ...
        let has_dg =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_PAIRING_DOWNGRADE);
        assert!(!has_dg, "first pairing should not alert");
    }

    #[test]
    fn pairing_upgrade_no_alert() {
        let mut mon = BleMonitor::new();
        // First pairing: JustWorks, Legacy.
        let _ = mon.inspect(&make_pair_complete(
            PEER_A,
            Some(PairingMethod::JustWorks),
            Some(false),
        ...
        // Upgrade to Numeric, Secure Connections.
        let r = mon.inspect(&make_pair_complete(
            PEER_A,
            Some(PairingMethod::Numeric),
            Some(true),
        ...
        let has_dg =
            (0..r.alert_count as usize).any(|i| r.alerts[i].source_id == ALERT_PAIRING_DOWNGRADE);
        assert!(!has_dg, "upgrade should not alert");
    }

    #[test]
    fn pairing_same_method_no_alert() {
        let mut mon = BleMonitor::new();
        let _ = mon.inspect(&make_pair_complete(
            PEER_A,
            Some(PairingMethod::Numeric),
    ...
    #[test]
    fn pairing_method_is_authenticated() {
        assert!(!PairingMethod::JustWorks.is_authenticated());
        assert!(PairingMethod::Passkey.is_authenticated());
        assert!(PairingMethod::Numeric.is_authenticated());
        assert!(PairingMethod::OutOfBand.is_authenticated());
    }
}
