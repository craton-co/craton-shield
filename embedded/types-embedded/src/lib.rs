...
/// [`vs_types::VsError`], [`vs_types::PayloadHash`]) without adding an
/// explicit dependency.
pub use vs_types;
...
/// Maximum number of in-flight `CoAP` block-wise transfer reassembly slots.
#[cfg(feature = "capacity-xl")]
pub const MAX_COAP_BLOCK_TRANSFERS: usize = 32;
/// Maximum number of in-flight `CoAP` block-wise transfer reassembly slots.
#[cfg(all(feature = "capacity-large", not(feature = "capacity-xl")))]
pub const MAX_COAP_BLOCK_TRANSFERS: usize = 16;
/// Maximum number of in-flight `CoAP` block-wise transfer reassembly slots.
#[cfg(not(any(feature = "capacity-large", feature = "capacity-xl")))]
pub const MAX_COAP_BLOCK_TRANSFERS: usize = 8;

/// Maximum number of distinct peers tracked for `CoAP` Observe registrations
/// (RFC 7641 flood protection).
#[cfg(feature = "capacity-xl")]
pub const MAX_COAP_OBSERVE_PEERS: usize = 64;
/// Maximum number of distinct peers tracked for `CoAP` Observe registrations.
#[cfg(all(feature = "capacity-large", not(feature = "capacity-xl")))]
pub const MAX_COAP_OBSERVE_PEERS: usize = 32;
/// Maximum number of distinct peers tracked for `CoAP` Observe registrations.
#[cfg(not(any(feature = "capacity-large", feature = "capacity-xl")))]
pub const MAX_COAP_OBSERVE_PEERS: usize = 16;

/// Maximum simultaneous Observe registrations a single peer may hold before
/// `ALERT_OBSERVE_FLOOD` fires.
pub const MAX_COAP_OBSERVE_PER_PEER: u16 = 32;
...
///
/// Marked `#[non_exhaustive]` so future MQTT versions can add packet types
/// (e.g. additional v5 control packets) without an MQTT-spec change becoming
/// a breaking change for downstream consumers of this foundation crate.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
...
/// MQTT `QoS` levels.
///
/// Marked `#[non_exhaustive]` for forward compatibility, even though the
/// MQTT spec only defines three levels today.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
pub enum MqttQoS {
...
/// `CoAP` method codes.
///
/// Marked `#[non_exhaustive]` because RFC extensions (e.g. RFC 8132 FETCH,
/// PATCH, iPATCH) can extend the method code space without a SemVer break.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
pub enum CoapMethod {
...
/// `CoAP` message types.
///
/// Marked `#[non_exhaustive]` for forward compatibility with future RFC
/// extensions to the message-type space.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
...
///
/// Follows RFC 7252 for message types and method codes, RFC 7959 for
/// block-wise transfers, and RFC 7641 for the Observe option.
#[derive(Debug, Clone, PartialEq)]
#[repr(C)]
pub struct CoapMessage {
    ...
    /// Timestamp in microseconds.
    pub timestamp_us: u64,
    /// Opaque peer identifier (e.g. a hash of the source IP:port, derived by
    /// the link-layer adapter). Used by the monitor for per-peer state
    /// tracking such as Observe-registration counts and block-wise transfer
    /// reassembly. `0` denotes "unknown peer" — monitors should treat all
    /// `0`-id messages as originating from one shared bucket.
    pub peer_id: u32,
    /// `true` if the link-layer adapter received this message over an
    /// authenticated secure channel (DTLS / OSCORE). `false` means clear-text
    /// `CoAP`. Used for the `CoapMonitor`'s DTLS-bypass check.
    pub secure_transport: bool,
    /// `true` if the message carried a Block1 option (RFC 7959 §2.3),
    /// describing the **request** body block index `block_num`. Block1 is
    /// used on requests (e.g. large PUT/POST payloads).
    pub has_block1: bool,
    /// `true` if the message carried a Block2 option (RFC 7959 §2.4),
    /// describing the **response** body block index `block_num` that the
    /// client is requesting/acknowledging.
    pub has_block2: bool,
    /// Zero-based block sequence number (`NUM` field of the Block1/Block2
    /// option). Only meaningful when `has_block1 || has_block2`.
    pub block_num: u32,
    /// `M` (More) bit: `true` while further blocks follow. The terminal
    /// block has `more == false`.
    pub block_more: bool,
    /// `SZX` (block size exponent), 0..=6 per RFC 7959 §2.2. Block payload
    /// size in bytes is `1 << (szx + 4)` (i.e. 16..=1024).
    pub block_szx: u8,
    /// `true` if the message carried an Observe option (RFC 7641). The
    /// numeric value is in `observe_value`.
    pub has_observe: bool,
    /// Observe option value: `0` = register, `1` = deregister (other values
    /// are notification sequence numbers carried on responses). Only
    /// meaningful when `has_observe == true`.
    pub observe_value: u32,
}
...
/// BLE pairing method used to establish a link.
///
/// Captured from SMP pairing feature exchange (Bluetooth Core Spec Vol 3,
/// Part H §2.3).  Allows the IDS to detect pairing-method downgrade attacks.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum PairingMethod {
    /// Just Works — no authentication, susceptible to MITM.
    JustWorks,
    /// Passkey Entry — six-digit passkey, authenticated.
    Passkey,
    /// Numeric Comparison — both sides display a number, authenticated
    /// (Secure Connections only).
    Numeric,
    /// Out-of-Band — keys exchanged via OOB channel (e.g. NFC).
    OutOfBand,
}

impl PairingMethod {
    /// Returns `true` if this pairing method provides MITM protection
    /// (i.e. is authenticated).  `JustWorks` is the only unauthenticated
    /// method.
    #[inline]
...
pub struct BleEvent {
    ...
    /// Timestamp in microseconds.
    pub timestamp_us: u64,
    /// Current ATT MTU when applicable (e.g. after MTU exchange or on
    /// GATT operations).  `None` when the event has no MTU context.
    pub att_mtu: Option<u16>,
    /// GATT attribute handle for read/write events.  Distinct from
    /// `conn_handle`, which identifies the connection itself.
    pub gatt_handle: Option<u16>,
    /// GATT permission flags reported by the stack for the operation.
    ///
    /// Bit 0: authenticated link (LE Secure Connections or authenticated
    /// pairing established).  Bit 1: encrypted link.  Bit 2: authorized
    /// (application-level authorization granted).  Remaining bits are
    /// implementation-defined and ignored by the IDS.
    pub gatt_perm_flags: Option<u8>,
    /// Pairing method used (captured on `PairingComplete` events).
    pub pairing_method: Option<PairingMethod>,
    /// Whether LE Secure Connections (vs Legacy pairing) was used
    /// (captured on `PairingComplete` events).
    pub pairing_secure_conn: Option<bool>,
}

/// Bit flag in `BleEvent::gatt_perm_flags`: the link is authenticated
/// (authenticated pairing or LE Secure Connections established).
pub const GATT_PERM_AUTHENTICATED: u8 = 0b0000_0001;

/// Bit flag in `BleEvent::gatt_perm_flags`: the link is encrypted.
pub const GATT_PERM_ENCRYPTED: u8 = 0b0000_0010;

/// Bit flag in `BleEvent::gatt_perm_flags`: application-level authorization
/// was granted for the operation.
pub const GATT_PERM_AUTHORIZED: u8 = 0b0000_0100;

/// BLE event types relevant for IDS.
///
/// Marked `#[non_exhaustive]` so additional BLE Core Spec events (e.g.
/// future GATT operations or LE feature events) can be added without a
/// SemVer break.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
...
/// Zigbee frame type.
///
/// Marked `#[non_exhaustive]` so additional IEEE 802.15.4 / Zigbee frame
/// types (e.g. multipurpose, extended) can be added without a SemVer break.
...
///
/// Marked `#[non_exhaustive]` so future `LoRaWAN` MAC versions can add
/// message types without a SemVer break.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
#[repr(u8)]
#[non_exhaustive]
...
///
/// Marked `#[non_exhaustive]` because Modbus function codes can be
/// extended (e.g. file-record access, encapsulated transports) without a
/// SemVer break.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
#[repr(u8)]
#[non_exhaustive]
...
/// # DoS Mitigation
///
/// Adversary-controlled inputs (MQTT topics, CoAP URIs) can be collision-flooded
/// into a single hash bucket. Operators MUST rate-limit per hash bucket as well
/// as per source/peer to bound worst-case work. This is already done by
/// `mqtt-monitor` and `coap-monitor`; downstream consumers building on this
/// hash directly should follow the same pattern.
#[inline]
...
/// # Effective Entropy
///
/// The 32-byte (256-bit) output is **misleading** as an entropy measure. The
/// underlying streams share a 32-bit state space and are correlated by
/// construction, so against an adversary who controls input the effective
/// collision resistance is closer to **~32-64 bits**, not 160 or 256. This
/// function is intended for **telemetry deduplication**, not security.
///
/// # Security Note
///
/// This is **NOT** a cryptographic hash and **MUST NOT** be used for integrity
...
pub fn compute_payload_hash(data: &[u8]) -> vs_types::PayloadHash {
    ...
    // TODO(perf): unroll 4x or process u32 chunks via chunks_exact(4) for Cortex-M4
    // to better amortise the per-byte multiply/rotate cost on 32-bit MCUs.
    for &byte in data {
        let b = byte as u32;
        // h1: standard FNV-1a
        h1 ^= b;
...
///
/// Stores up to 8 key-value pairs with fixed maximum
/// sizes (32 byte keys, 256 byte values). Data is stored in stack-allocated arrays with no heap usage.
///
/// # Intended Use
///
...
impl PersistenceProvider for InMemoryPersistenceProvider {
    fn save(&mut self, key: &[u8], data: &[u8]) -> Result<(), vs_types::VsError> {
        ...
        if let Some(idx) = self.find_key(key) {
            ...
            self.values[idx][data.len()..PERSISTENCE_VAL_MAX].fill(0);
        ...
        self.keys[idx][key.len()..].fill(0);
        self.key_lens[idx] = key.len() as u8;
        self.values[idx][..data.len()].copy_from_slice(data);
        self.values[idx][data.len()..].fill(0);
        self.value_lens[idx] = data.len() as u16;
...
///
/// Marked `#[non_exhaustive]` so new audit categories can be added without
/// a SemVer break for downstream consumers.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
...
///
/// Marked `#[non_exhaustive]` so future Zigbee Trust Center protocol events
...
///
/// Marked `#[non_exhaustive]` so that new address families (e.g. domain
/// names, tunneling envelopes) can be added in the future without a
/// SemVer break.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum IpAddress {
...
///
/// Marked `#[non_exhaustive]` so additional actions (e.g. quarantine,
/// rate-limit, log-only) can be added in the future without a SemVer
/// break.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum IpAction {
...
impl ModbusIpFilter {
    ...
    /// # Timing
    ///
    /// Runs in constant time with respect to the prefix length, address family,
    /// and address bytes. The family check and `prefix_len == 0` short-circuit
    /// are restructured so the masked comparison executes unconditionally;
    /// failure cases are folded into the accumulated result. `core::hint::black_box`
    /// is used to defeat optimizer short-circuiting.
    #[must_use = "IP filter result must not be silently ignored"]
    #[allow(clippy::arithmetic_side_effects)]
    pub fn matches(&self, addr: &[u8; 4]) -> bool {
        // Extract IPv4 bytes regardless of family, defaulting to zeros for V6
        // so the masked comparison below runs in constant time.
        let (filter_bytes, family_ok) = match &self.ip {
            IpAddress::V4(v4) => (*v4, true),
            IpAddress::V6(_) => ([0u8; 4], false),
        };

        // Clamp prefix to [0, 32]; `prefix_len == 0` becomes "match any" via
        // a mask of 0, so we never short-circuit on that case either.
        let mask_bits = if self.prefix_len >= 32 {
            32u32
        } else {
    ...
    /// Runs in constant time with respect to the prefix length, address family,
    /// and address bytes. The family check and `prefix_len == 0` short-circuit
    /// are folded into the accumulated result so the byte-wise comparison
    /// always executes for all 16 bytes. `core::hint::black_box` is used to
    /// defeat optimizer short-circuiting.
    #[must_use = "IP filter result must not be silently ignored"]
    #[allow(clippy::arithmetic_side_effects)]
    pub fn matches_v6(&self, addr: &[u8; 16]) -> bool {
        // Extract IPv6 bytes regardless of family, defaulting to zeros for V4
        // so the masked comparison below runs in constant time.
        let (filter_bytes, family_ok) = match &self.ip {
            IpAddress::V6(v6) => (*v6, true),
            IpAddress::V4(_) => ([0u8; 16], false),
        };

        // Clamp prefix to [0, 128].
        let prefix = if self.prefix_len > 128 {
            128
        } else {
            self.prefix_len
        };
        ...
        // Always iterate all 16 bytes; gate each byte's contribution by a mask
        // derived from its position relative to `full_bytes`.
        let mut diff: u8 = 0;
        let mut i = 0;
        while i < 16 {
            // mask = 0xFF for bytes fully inside the prefix; 0 otherwise.
            // Constructed branchlessly so the loop body is uniform.
            let in_full = (i < full_bytes) as u8;
            let full_mask = 0u8.wrapping_sub(in_full); // 0xFF if in_full, 0 otherwise.

            // Partial-byte mask applies only at index == full_bytes when
            // remaining_bits > 0.
            let at_partial = ((i == full_bytes) & (remaining_bits > 0)) as u8;
            let partial_mask_shift = 8u32.wrapping_sub(remaining_bits as u32) & 0x07;
            let partial_mask_val: u8 = if remaining_bits == 0 {
                0
            } else {
                0xFF_u8 << partial_mask_shift
            };
            let partial_mask = partial_mask_val & 0u8.wrapping_sub(at_partial);

            let byte_mask = full_mask | partial_mask;
            diff |= (filter_bytes[i] ^ addr[i]) & byte_mask;
            i += 1;
        }
...
///
/// Marked `#[non_exhaustive]` because the Modbus exception code space can
/// be extended (e.g. gateway exceptions 0x0A/0x0B) without a SemVer break.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
...
///
/// Marked `#[non_exhaustive]` so additional BLE Core Spec address types
/// can be added without a SemVer break.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u8)]
#[non_exhaustive]
...
mod tests {
    ...
    fn ble_event_default() {
        ...
        assert!(evt.att_mtu.is_none());
        assert!(evt.gatt_perm_flags.is_none());
        assert!(evt.pairing_method.is_none());
        assert!(evt.pairing_secure_conn.is_none());
    }
    ...
    #[test]
    fn gatt_perm_flag_constants_distinct() {
        assert_ne!(GATT_PERM_AUTHENTICATED, GATT_PERM_ENCRYPTED);
        assert_ne!(GATT_PERM_AUTHENTICATED, GATT_PERM_AUTHORIZED);
        assert_ne!(GATT_PERM_ENCRYPTED, GATT_PERM_AUTHORIZED);
    }
