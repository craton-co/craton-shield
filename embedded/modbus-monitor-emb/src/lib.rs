...
const ALERT_BROADCAST_WRITE: u32 = 12;
const ALERT_LENGTH_MISMATCH: u32 = 13;

// ---------------------------------------------------------------------------
// Broadcast handling
// ---------------------------------------------------------------------------

/// Synthetic rate-bucket key reserved for broadcast (`unit_id == 0`) traffic.
///
/// The Modbus unit ID space is 0..=255 and `0` is the broadcast address.
/// We use `0` as the bucket key itself; the bucket allocator already treats
/// the key as opaque, so this is unambiguous as long as no per-unit rule for
/// `0` is in play (broadcast is never rule-matched — it is policed globally).
const BROADCAST_BUCKET_KEY: u8 = 0;

/// Default rate cap (requests/sec) applied to broadcast (`unit_id == 0`)
/// traffic. Broadcast is policed globally regardless of per-unit rules to
/// prevent a single broadcast frame from bypassing per-unit policies.
const BROADCAST_RATE_PER_SEC: u16 = 10;
...
impl RateBucket {
    ...
    fn try_consume(&mut self, now_us: u64) -> bool {
        // Cap `elapsed` at 1_000_000 microseconds (1 second) before the
        // multiply. The refill is `min(capacity, elapsed * capacity / 1e6)`,
        // so any `elapsed >= 1e6` saturates the bucket; capping here keeps
        // the multiplicand bounded so the divide folds on 32-bit MCUs.
        let elapsed = now_us.saturating_sub(self.last_refill_us).min(1_000_000);
...
// ---------------------------------------------------------------------------
// Per-FC PDU length validation
// ---------------------------------------------------------------------------
//
// Modbus PDU layout (per Modbus Application Protocol Specification V1.1b3):
//
//   * FC 1, 2, 3, 4 (read requests):
//       PDU = FC(1) + start_addr(2) + qty(2) = 5 bytes
//   * FC 5 (write single coil), FC 6 (write single register):
//       PDU = FC(1) + addr(2) + value(2) = 5 bytes
//   * FC 15 (write multiple coils):
//       PDU = FC(1) + addr(2) + qty(2) + byte_count(1) + coil_bytes(ceil(qty/8))
//   * FC 16 (write multiple registers):
//       PDU = FC(1) + addr(2) + qty(2) + byte_count(1) + register_bytes(2*qty)
//
// For TCP MBAP, `mbap_length = unit_id_byte_count(1) + pdu_byte_count`.
//
// We return `None` for `ModbusFunction::Unknown` — Unknown is policed by the
// separate `ALERT_UNKNOWN_FUNCTION` path and never reaches this helper.

/// Returns the expected request-PDU byte count for `function` with the given
/// `quantity`, or `None` if the function code is `Unknown` or otherwise not
/// covered by length validation.
///
/// # Covered function codes
///
/// * `FC 0x01` (`ReadCoils`)
/// * `FC 0x02` (`ReadDiscreteInputs`)
/// * `FC 0x03` (`ReadHoldingRegisters`)
/// * `FC 0x04` (`ReadInputRegisters`)
/// * `FC 0x05` (`WriteSingleCoil`)
/// * `FC 0x06` (`WriteSingleRegister`)
/// * `FC 0x0F` (`WriteMultipleCoils`)
/// * `FC 0x10` (`WriteMultipleRegisters`)
///
/// # Skip-validation semantics
///
/// All other function codes — including `FC 0x07` (Read Exception Status),
/// `FC 0x08` (Diagnostics), `FC 0x0B` (Get Comm Event Counter),
/// `FC 0x0C` (Get Comm Event Log), `FC 0x11` (Report Server ID),
/// `FC 0x14`/`FC 0x15` (Read/Write File Record), `FC 0x16` (Mask Write
/// Register), `FC 0x17` (Read/Write Multiple Registers), `FC 0x18` (Read
/// FIFO Queue), and `FC 0x2B` (Encapsulated Interface Transport) — return
/// `None`. The caller (`inspect_inner`) interprets `None` as "skip PDU
/// length validation"; these codes bypass the per-FC length check and are
/// policed only by the unit-rule, function-policy, and rate-limit paths.
///
/// `Unknown` likewise returns `None` but is handled separately by the
/// `ALERT_UNKNOWN_FUNCTION` path before this helper is consulted.
#[inline]
fn expected_request_pdu_len(function: ModbusFunction, quantity: u16) -> Option<u16> {
    match function {
        ...
        | ModbusFunction::WriteSingleRegister => Some(5),
        ModbusFunction::WriteMultipleCoils => {
            // PDU = 1 + 2 + 2 + 1 + ceil(qty/8) = 6 + ceil(qty/8)
            let coil_bytes = (quantity as u32 + 7) / 8;
            Some(6_u16.saturating_add(coil_bytes.min(u16::MAX as u32) as u16))
        }
        ModbusFunction::WriteMultipleRegisters => {
            // PDU = 1 + 2 + 2 + 1 + 2*qty = 6 + 2*qty
            let reg_bytes = (quantity as u32).saturating_mul(2);
            Some(6_u16.saturating_add(reg_bytes.min(u16::MAX as u32) as u16))
        }
        ModbusFunction::Unknown => None,
        _ => None,
    }
}
...
impl ModbusMonitor {
    ...
    fn inspect_inner(
        ...
        //
        // Note: subsequent calls return the cached value in O(1) without
        // re-hashing — the hash is computed at most once per `inspect_inner`
        // call regardless of how many alerts are emitted.
        ...
        let clock_anomalous = !self.timestamp_validator.validate(ts_us);
        if clock_anomalous {
            result.push_alert(
                AlertSeverity::Low,
                source_type,
        ...
        if let Some(ip) = src_ip.filter(|_| self.ip_filter_count > 0) {
            ...
            // Exact-match prefix length (/32 for IPv4, /128 for IPv6). Once
            // we see an exact match, no longer-prefix entry can exist so we
            // can short-circuit the remaining slots.
            let exact_plen: u8 = match ip {
                IpAddress::V4(_) => 32,
                IpAddress::V6(_) => 128,
                // IpAddress is #[non_exhaustive]; future variants disable short-circuit.
                _ => u8::MAX,
            };
            for i in 0..MAX_IP_FILTERS {
        ...
        // Per-FC length validation (Modbus ADU/PDU).
        //
        // For TCP MBAP, `length = unit_id_byte_count(1) + pdu_byte_count`.
        // For RTU, the PDU is a fixed function of (FC, quantity) — see
        // `expected_request_pdu_len`.
        //
        // We only validate when `payload_len > 0` (callers that have not
        // populated the field are treated as opting out); if a length is
        // supplied it MUST match the spec-mandated value or we reject.
        if payload_len > 0 {
            if let Some(expected) = expected_request_pdu_len(function, quantity) {
                if payload_len != expected as usize {
                    result.allowed = false;
                    result.push_alert(
                        AlertSeverity::Medium,
                        source_type,
                        ALERT_LENGTH_MISMATCH,
                        ts_us,
                        self.next_alert_id(),
                        make_fingerprint(),
                    );
        ...
        // Broadcast (unit_id == 0) policy.
        //
        // Broadcast traffic bypasses the per-unit rule table and therefore
        // every per-unit ReadOnly/range/rate policy. We police broadcast
        // GLOBALLY here: writes are denied (ReadOnly), and a synthetic
        // per-broadcast rate bucket is applied so a single broadcast frame
        // cannot circumvent per-unit policies.
        if unit_id == 0 {
            // Enforce ReadOnly for broadcast: writes to the broadcast address
            // would propagate to every device on the bus.
            if function.is_write() {
                result.allowed = false;
                result.push_alert(
                    AlertSeverity::High,
                    source_type,
                    ALERT_BROADCAST_WRITE,
                    ts_us,
                    self.next_alert_id(),
                    make_fingerprint(),
                );
            ...
            // Apply global broadcast rate bucket using a synthetic key.
            // Skip on clock anomaly to prevent attacker-controlled `ts_us`
            // from poisoning the bucket via a spoofed forward jump.
            if !clock_anomalous
                && !self.rate_limit_check(BROADCAST_BUCKET_KEY, BROADCAST_RATE_PER_SEC, ts_us)
            {
                result.push_alert(
                    AlertSeverity::Medium,
                    source_type,
        ...
        // Unit rule matching. First-match-wins, so break after the first hit
        // to avoid scanning the remaining `rule_count - i - 1` entries.
        ...
        if let Some(idx) = matched {
            ...
            // Rate limiting.
            //
            // Skip the bucket update entirely on a clock anomaly. An attacker
            // who controls `ts_us` could otherwise spoof a large forward jump
            // and refill the bucket via `try_consume`'s elapsed-time math,
            // defeating the rate cap. We still surfaced the anomaly above; we
            // simply do not let the spoofed timestamp poison bucket state.
            let max_rate = self.rules[idx].max_rate_per_sec;
            if max_rate > 0 && !clock_anomalous && !self.rate_limit_check(unit_id, max_rate, ts_us)
            {
...
mod tests {
    ...
    fn make_rtu(unit: u8, func: ModbusFunction, addr: u16, qty: u16, ts: u64) -> ModbusRtuMessage {
        // Default to the spec-mandated PDU length so messages pass the
        // per-FC length validation. Tests that exercise the validator
        // explicitly overwrite `payload_len` after construction.
        let payload_len = expected_request_pdu_len(func, qty).unwrap_or(0);
        ModbusRtuMessage {
            unit_id: unit,
            function: func,
            register_addr: addr,
    ...
    // -----------------------------------------------------------------------
    // Regression: broadcast (unit_id=0) bypassing per-unit policies.
    //
    // Before the fix: a unit_id=0 frame skipped the per-unit rule lookup
    // (rules are keyed by non-zero unit_id) and therefore bypassed every
    // ReadOnly / register-range / rate-limit policy installed for unit 0.
    // After the fix: broadcast is policed GLOBALLY — writes are denied
    // outright (ReadOnly), and a synthetic broadcast rate bucket applies.
    // -----------------------------------------------------------------------

    #[test]
    fn broadcast_write_is_denied_globally() {
        // No per-unit rule for 0 — broadcast policy must still apply.
        let mut mon = ModbusMonitor::new();
        let msg = make_rtu(0, ModbusFunction::WriteSingleCoil, 0, 1, 1_000_000);
        let r = mon.inspect_rtu(&msg);
        assert!(!r.allowed, "broadcast write must be blocked");
        assert!(r.alert_count > 0);
        assert_eq!(r.alerts[0].source_id, ALERT_BROADCAST_WRITE);
        assert_eq!(r.alerts[0].severity, AlertSeverity::High);
    }

    #[test]
    fn broadcast_write_denied_even_when_explicit_allow_rule_for_unit_zero() {
        // Attacker can't install an Allow rule for unit 0 to bypass the
        // global broadcast policy — broadcast is checked BEFORE rule match.
        let mut mon = ModbusMonitor::new();
        mon.add_rule(0, UnitAction::Allow, FunctionPolicy::Any, 0, u16::MAX, 0)
            .unwrap();
        let msg = make_rtu(0, ModbusFunction::WriteMultipleRegisters, 0, 1, 1_000_000);
        let r = mon.inspect_rtu(&msg);
        assert!(!r.allowed, "broadcast write must be blocked globally");
        assert_eq!(r.alerts[0].source_id, ALERT_BROADCAST_WRITE);
    }

    // -----------------------------------------------------------------------
    // Regression: clock anomaly must not refill the rate bucket.
    //
    // Before the fix: `validate(ts_us) -> false` produced an alert but
    // execution still fell through to `rate_limit_check(unit_id, _, ts_us)`,
    // where the bucket's `try_consume` computes
    //   refill = elapsed * capacity / 1e6
    // From a forward jump well beyond MAX_CLOCK_FORWARD_JUMP_US this fully
    // refills the bucket, defeating the rate limiter.
    // After the fix: on clock anomaly we still alert but skip the rate-
    // bucket update entirely so the attacker cannot poison bucket state.
    // -----------------------------------------------------------------------

    #[test]
    fn clock_anomaly_does_not_refill_rate_bucket() {
        let mut mon = ModbusMonitor::new();
        // 2 req/sec cap.
        mon.add_rule(1, UnitAction::Allow, FunctionPolicy::Any, 0, u16::MAX, 2)
            .unwrap();
        ...
        let _ = mon.inspect_rtu(&make_rtu(1, ModbusFunction::ReadCoils, 0, 10, 1_000_000));
        // Consume the second token at a normal timestamp.
        let _ = mon.inspect_rtu(&make_rtu(1, ModbusFunction::ReadCoils, 0, 10, 1_100_000));
        // Bucket is now empty (capacity=2, consumed=2).
        // Attacker sends a far-future timestamp (> MAX_CLOCK_FORWARD_JUMP_US
        // = 1 hour). Without the fix, this would refill the bucket and
        // immediately allow another request without a rate alert.
        let attack_ts = 1_100_000_u64 + 10 * 3_600_000_000_u64; // +10 hours
        let r = mon.inspect_rtu(&make_rtu(1, ModbusFunction::ReadCoils, 0, 10, attack_ts));
        // Must surface a timestamp anomaly alert (the attack itself).
        let mut saw_ts_anomaly = false;
        for i in 0..r.alert_count as usize {
            if r.alerts[i].source_id == ALERT_TIMESTAMP_ANOMALY {
                saw_ts_anomaly = true;
            }
        }
        assert!(saw_ts_anomaly, "spoofed forward-jump must alert");
        // CRITICAL: the rate bucket must NOT have been refilled. Therefore
        // a follow-up request with a normal timestamp must still observe
        // the bucket empty and produce a rate-limit alert.
        let r2 = mon.inspect_rtu(&make_rtu(1, ModbusFunction::ReadCoils, 0, 10, 1_200_000));
        let mut saw_rate_alert_after = false;
        for i in 0..r2.alert_count as usize {
            if r2.alerts[i].source_id == ALERT_RATE_LIMITED {
                saw_rate_alert_after = true;
            }
        }
        assert!(
            saw_rate_alert_after,
            "bucket must still be drained after clock-anomaly attempt"
        );
    }

    #[test]
    fn clock_anomaly_skips_broadcast_rate_bucket_too() {
        // Same poisoning protection must apply to the synthetic broadcast
        // bucket.
        let mut mon = ModbusMonitor::new();
        // Drain the broadcast bucket.
        for i in 0..BROADCAST_RATE_PER_SEC {
            let _ = mon.inspect_rtu(&make_rtu(
                0,
                ModbusFunction::ReadCoils,
                0,
                10,
        ...
        // Bucket empty. Spoof a forward jump and verify it neither refills
        // nor produces a rate alert (rate check is skipped, not failed).
        let attack_ts = 1_000_000_u64 + 10 * 3_600_000_000_u64;
        let r = mon.inspect_rtu(&make_rtu(0, ModbusFunction::ReadCoils, 0, 10, attack_ts));
        let mut saw_rate_alert = false;
        let mut saw_ts_anomaly = false;
        for i in 0..r.alert_count as usize {
            if r.alerts[i].source_id == ALERT_RATE_LIMITED {
                saw_rate_alert = true;
        ...
        assert!(saw_ts_anomaly);
        assert!(!saw_rate_alert, "spoofed ts must skip broadcast bucket too");
    }

    #[test]
    fn broadcast_read_passes_but_is_rate_limited() {
        // Reads to broadcast are allowed but a synthetic per-broadcast
        // rate bucket applies. Send BROADCAST_RATE_PER_SEC + 1 reads in a
        // tight window and the last one must produce a rate alert.
        let mut mon = ModbusMonitor::new();
        for i in 0..BROADCAST_RATE_PER_SEC {
            let msg = make_rtu(0, ModbusFunction::ReadCoils, 0, 10, 1_000_000 + i as u64);
            let r = mon.inspect_rtu(&msg);
            assert!(r.allowed);
            assert_eq!(
                r.alert_count, 0,
                "broadcast reads under the cap should be alert-free"
            );
        }
        // Next read in the same microsecond window — bucket is empty.
        let msg = make_rtu(
            0,
            ModbusFunction::ReadCoils,
            0,
            10,
            1_000_000 + BROADCAST_RATE_PER_SEC as u64,
        );
        let r = mon.inspect_rtu(&msg);
        assert!(r.allowed, "rate-limited broadcast is allowed but alerted");
        let mut saw_rate_alert = false;
        for i in 0..r.alert_count as usize {
            if r.alerts[i].source_id == ALERT_RATE_LIMITED {
                saw_rate_alert = true;
        ...
        assert!(saw_rate_alert, "broadcast must produce rate-limit alert");
    }

    // -----------------------------------------------------------------------
    // Regression: ADU/PDU length validation.
    //
    // Modbus PDU lengths are fixed per FC (reads, single-writes) or follow a
    // deterministic formula of `quantity` (multi-writes). For TCP MBAP the
    // length field MUST equal `unit_id_byte_count(1) + pdu_byte_count`.
    // Without per-FC length validation, an attacker can ship a malformed
    // frame past the allowlist and reach downstream parsers with a length
    // that disagrees with the FC.
    // -----------------------------------------------------------------------

    #[test]
    fn length_mismatch_read_coils_is_rejected() {
        // ReadCoils PDU is always 5 bytes; supplying 99 must reject.
        let mut mon = ModbusMonitor::new();
        let mut msg = make_rtu(1, ModbusFunction::ReadCoils, 0, 10, 1_000_000);
        msg.payload_len = 99;
        let r = mon.inspect_rtu(&msg);
        assert!(!r.allowed);
        assert_eq!(r.alerts[0].source_id, ALERT_LENGTH_MISMATCH);
        assert_eq!(r.alerts[0].severity, AlertSeverity::Medium);
    }

    #[test]
    fn length_mismatch_write_multiple_registers_is_rejected() {
        // FC 16 with qty=4 ⇒ PDU = 6 + 2*4 = 14 bytes. 13 must reject.
        let mut mon = ModbusMonitor::new();
        let mut msg = make_rtu(1, ModbusFunction::WriteMultipleRegisters, 0, 4, 1_000_000);
        msg.payload_len = 13;
        let r = mon.inspect_rtu(&msg);
        assert!(!r.allowed);
        assert_eq!(r.alerts[0].source_id, ALERT_LENGTH_MISMATCH);
    ...
    #[test]
    fn length_mismatch_write_multiple_coils_is_rejected() {
        // FC 15 with qty=10 ⇒ ceil(10/8)=2 ⇒ PDU = 6 + 2 = 8. 7 must reject.
        let mut mon = ModbusMonitor::new();
        let mut msg = make_rtu(1, ModbusFunction::WriteMultipleCoils, 0, 10, 1_000_000);
        msg.payload_len = 7;
        let r = mon.inspect_rtu(&msg);
        assert!(!r.allowed);
        assert_eq!(r.alerts[0].source_id, ALERT_LENGTH_MISMATCH);
    ...
    #[test]
    fn length_correct_passes_for_each_fc() {
        let mut mon = ModbusMonitor::new();
        let cases: &[(ModbusFunction, u16, u16)] = &[
            (ModbusFunction::ReadCoils, 10, 5),
            (ModbusFunction::ReadDiscreteInputs, 10, 5),
            (ModbusFunction::ReadHoldingRegisters, 10, 5),
            (ModbusFunction::ReadInputRegisters, 10, 5),
            (ModbusFunction::WriteSingleCoil, 1, 5),
            (ModbusFunction::WriteSingleRegister, 1, 5),
            (ModbusFunction::WriteMultipleCoils, 16, 6 + 2), // ceil(16/8)=2
            (ModbusFunction::WriteMultipleRegisters, 5, 6 + 10), // 2*5=10
        ];
        for (fc, qty, expected) in cases {
            let mut msg = make_rtu(1, *fc, 0, *qty, 1_000_000);
            msg.payload_len = *expected;
            let r = mon.inspect_rtu(&msg);
            assert!(
                r.allowed,
                "fc={:?} qty={} expected_pdu={} must be allowed",
                fc, qty, expected
            );
        }
    }
    ...
    #[test]
    fn length_zero_opts_out() {
        // payload_len == 0 is treated as "not asserted" so callers that
        // don't supply length info don't break. (Default helper now sets
        // a correct length; this is the explicit opt-out path.)
        let mut mon = ModbusMonitor::new();
        let mut msg = make_rtu(1, ModbusFunction::ReadCoils, 0, 10, 1_000_000);
        msg.payload_len = 0;
        let r = mon.inspect_rtu(&msg);
    ...
    #[test]
    fn length_mismatch_alerts_on_tcp_too() {
        // For TCP, the MBAP length equals unit_id(1) + pdu_bytes; a wrong
        // PDU length surfaces the same ALERT_LENGTH_MISMATCH path.
        let mut mon = ModbusMonitor::new();
        let mut tcp = make_tcp(1, ModbusFunction::ReadHoldingRegisters, 0, 10, 1_000_000);
        tcp.rtu.payload_len = 200; // wrong: expected 5
        let r = mon.inspect_tcp(&tcp);
        assert!(!r.allowed);
        assert_eq!(r.alerts[0].source_id, ALERT_LENGTH_MISMATCH);
        assert_eq!(r.alerts[0].source_type, SOURCE_MODBUS_TCP);
    }
    ...
    #[test]
    fn length_broadcast_not_bypass() {
        // Length check runs BEFORE the broadcast branch — a malformed
        // broadcast frame must still be rejected on length.
        let mut mon = ModbusMonitor::new();
        let mut msg = make_rtu(0, ModbusFunction::ReadCoils, 0, 10, 1_000_000);
        msg.payload_len = 99; // wrong: expected 5
        let r = mon.inspect_rtu(&msg);
        assert!(!r.allowed);
        assert_eq!(r.alerts[0].source_id, ALERT_LENGTH_MISMATCH);
    }
