...
//!
//! The `EmbeddedShield` orchestrator, its `submit_*` methods, and the
//! `configure_*` closures form the v1.0 stable surface and are governed by
//! `DEPRECATION.md`. The test-only `*_monitor_mut` accessors,
//! `core_mut`, and `drain_recent_alerts` are hidden behind `#[cfg(test)]`
//! and are NOT part of the stable surface.
...
/// Re-export of the BLE monitor crate for callers that want types
/// (`MacAction`, etc.) without an explicit `vs-ble-monitor` dependency.
pub use vs_ble_monitor;
/// Re-export of the `CoAP` monitor crate.
...
/// [`vs_runtime::PlatformConfig`] for callers who already import the
/// embedded `PlatformConfig` and want a non-shadowed name for the core one.
pub use vs_runtime::{self, PlatformConfig as CoreConfig};
/// Re-export of the embedded types crate (`MqttMessage`, `BleEvent`, etc.).
pub use vs_types_embedded;
/// Re-export of the Zigbee monitor crate.
...
/// Build a `const SENTINEL: SecurityAlert` with the requested `source_type`.
macro_rules! sentinel_for {
    ($source:expr) => {
        SecurityAlert {
            id: 0,
            severity: vs_types::AlertSeverity::Info,
...
// Pre-baked "blocked" (denied, zero-alert) inspect results, one per protocol.
//
// The previous implementation rebuilt a 4-element `[SecurityAlert; 4]` on
// every blocked submit call (~one per uninitialized/zero-timestamp call).
// Each blocked result is now a `const` and the helpers simply clone it,
// which is a cheap memcpy of a layout-known struct.

const BLOCKED_MQTT: MqttInspectResult = MqttInspectResult {
    allowed: false,
    alert_count: 0,
    alerts: [sentinel_for!(vs_types_embedded::SOURCE_MQTT); 4],
    alerts_dropped: 0,
};

const BLOCKED_COAP: CoapInspectResult = CoapInspectResult {
    allowed: false,
    alert_count: 0,
    alerts: [sentinel_for!(vs_types_embedded::SOURCE_COAP); 4],
    alerts_dropped: 0,
    reject_reason: None,
};

const BLOCKED_BLE: BleInspectResult = BleInspectResult {
    allowed: false,
    alert_count: 0,
    alerts: [sentinel_for!(vs_types_embedded::SOURCE_BLE); 4],
    alerts_dropped: 0,
};

const BLOCKED_ZIGBEE: ZigbeeInspectResult = ZigbeeInspectResult {
    allowed: false,
    alert_count: 0,
    alerts: [sentinel_for!(vs_types_embedded::SOURCE_ZIGBEE); 4],
    alerts_dropped: 0,
};

const BLOCKED_LORA: LoraInspectResult = LoraInspectResult {
    allowed: false,
    alert_count: 0,
    alerts: [sentinel_for!(vs_types_embedded::SOURCE_LORA); 4],
    alerts_dropped: 0,
};

const BLOCKED_MODBUS_RTU: ModbusInspectResult = ModbusInspectResult {
    allowed: false,
    alert_count: 0,
    alerts: [sentinel_for!(vs_types_embedded::SOURCE_MODBUS_RTU); 4],
    alerts_dropped: 0,
};

const BLOCKED_MODBUS_TCP: ModbusInspectResult = ModbusInspectResult {
    allowed: false,
    alert_count: 0,
    alerts: [sentinel_for!(vs_types_embedded::SOURCE_MODBUS_TCP); 4],
    alerts_dropped: 0,
};
...
/// - `$ts_assign`: an expression that mutates `$msg`'s timestamp field
///   (only executed AFTER the initialization and timestamp guards pass so
///   that a caller's message is not mutated when the call is rejected).
...
macro_rules! submit_body {
    ($self:ident, $monitor:ident, $inspect_method:ident, $msg:expr, $ts_us:expr, $ts_assign:expr, $blocked_fn:expr, $monitor_bit:expr) => {{
        ...
        // Guards passed — only now mutate the caller-provided message
        // timestamp.  Mutating before the guards would corrupt the caller's
        // message on a rejected call.
        $ts_assign;
        ...
        for i in 0..n {
            ...
            $self.store_recent_alert(&result.alerts[i], $ts_us);
        }
        result
    }};
}
...
impl<C: CryptoProvider + Clone, CB: AlertCallback> EmbeddedShield<C, CB> {
    ...
    /// must not panic. In `panic=abort` builds (the standard `no_std`
    /// configuration), a panicking callback aborts the process; in
    /// `panic=unwind` configurations it unwinds through the monitor, which
    /// may leave the runtime in an inconsistent state.
    ...
    ///
    /// The `ts_us` parameter is the caller-validated timestamp from the
    /// originating `submit_*` call.  It is used to drive the callback
    /// throttle window rather than `alert.timestamp_us`, which is set by
    /// the monitor and may not match the validated submit-call timestamp.
    ///
    /// The callback is suppressed when the timestamp fails monotonicity
    /// validation (see [`TimestampValidator`]), as a suspicious clock value
    /// could be used to manipulate the sliding-window throttle.
    #[inline]
    fn store_recent_alert(&mut self, alert: &SecurityAlert, ts_us: u64) {
        ...
        // throttle window.
        if ts_us == 0 || !self.callback_ts_validator.validate(ts_us) {
            return;
        }
        ...
        let jump = ts_us.saturating_sub(self.callback_window_start_us);
        if jump > CALLBACK_THROTTLE_WINDOW_US {
            self.callback_window_start_us = ts_us;
        ...
        if self.callback_window_count <= MAX_CALLBACK_BURST {
            self.alert_callback.on_alert(alert, ts_us);
        }
    }
    ...
    /// Submit a `CoAP` message for inspection (no amplification tracking).
    ///
    /// Returns a clean result if the system has been shut down.
    ///
    /// # Amplification tracking
    ///
    /// This method does **not** record the request for amplification
    /// detection: that path now requires a peer endpoint to prevent
    /// cross-peer correlation attacks. Use
    /// [`submit_coap_message_with_peer`](Self::submit_coap_message_with_peer)
    /// together with
    /// [`check_coap_amplification_with_peer`](Self::check_coap_amplification_with_peer)
    /// when amplification detection is required.
    ///
    /// # Timestamp Requirements
    ///
    /// `ts_us` **MUST** originate from a synchronized clock source (NTP,
    ...
    /// Submit a `CoAP` message for inspection, binding the request to a
    /// peer endpoint so amplification detection can correlate the response.
    ///
    /// Pair with
    /// [`check_coap_amplification_with_peer`](Self::check_coap_amplification_with_peer).
    pub fn submit_coap_message_with_peer(
        &mut self,
        msg: &mut CoapMessage,
        peer: &CoapPeer,
        ts_us: u64,
        ...
        // Guards passed — only now mutate the caller-provided message.
        msg.timestamp_us = ts_us;
        self.active_monitors |= MONITOR_COAP;
        let result = self.coap_monitor.inspect_with_peer(msg, peer);
        self.health_dirty = true;
        let n = core::cmp::min(result.alert_count as usize, result.alerts.len());
        for i in 0..n {
            self.core.route_alert(&result.alerts[i], ts_us);
    ...
    /// Check for `CoAP` amplification attack on a response — **legacy API**.
    ///
    /// Always returns `None`: amplification matching now requires a peer
    /// endpoint (see
    /// [`check_coap_amplification_with_peer`](Self::check_coap_amplification_with_peer)).
    #[deprecated(
        since = "0.7.0",
        note = "use check_coap_amplification_with_peer; legacy API cannot bind to a peer"
    )]
    pub fn check_coap_amplification(
        &mut self,
        _message_id: u16,
        _token: &[u8],
        _response_payload_len: u16,
        _ts_us: u64,
    ) -> Option<SecurityAlert> {
        None
    }

    /// Check for a `CoAP` amplification attack on a response, scoped to a
    /// specific peer endpoint. Correlates against entries previously recorded
    /// via
    /// [`submit_coap_message_with_peer`](Self::submit_coap_message_with_peer).
    pub fn check_coap_amplification_with_peer(
        &mut self,
        message_id: u16,
        token: &[u8],
        peer: &CoapPeer,
        ...
        let alert = self.coap_monitor.check_amplification_with_peer(
            message_id,
            token,
            peer,
        ...
        if let Some(ref a) = alert {
            ...
            self.store_recent_alert(a, ts_us);
        }
        alert
    }
    ...
    /// Test-only mutable accessor for the core runtime.
    ///
    /// Direct mutation bypasses alert routing, health tracking, and audit
    /// logging — production code must use the typed `submit_*` methods and
    /// configuration accessors. Marked `#[cfg(test)]` so it is not part of
    /// the stable v1.0 public API.
    #[cfg(test)]
    ...
    ///
    /// Use this instead of direct `mqtt_monitor_mut` access. The closure
    ...
    ///
    /// Use this instead of direct `coap_monitor_mut` access. The closure
    /// receives a mutable reference to the monitor and should return `Ok(())`
    /// on success.
    ...
    ///
    /// Use this instead of direct `ble_monitor_mut` access. The closure
    /// receives a mutable reference to the monitor and should return `Ok(())`
    /// on success.
    ...
    ///
    /// Use this instead of direct `zigbee_monitor_mut` access. The closure
    /// receives a mutable reference to the monitor and should return `Ok(())`
    /// on success.
    ...
    ///
    /// Use this instead of direct `lora_monitor_mut` access. The closure
    /// receives a mutable reference to the monitor and should return `Ok(())`
    /// on success.
    ...
    ///
    /// Use this instead of direct `modbus_monitor_mut` access. The closure
    /// receives a mutable reference to the monitor and should return `Ok(())`
    /// on success.
    ...
    /// Test-only mutable accessor for the MQTT monitor.
    ///
    /// Production code must use [`Self::configure_mqtt`] so audit logging is
    /// preserved. Hidden behind `#[cfg(test)]` to keep it out of the v1.0
    /// stable surface.
    #[cfg(test)]
    ...
    /// Test-only mutable accessor for the `CoAP` monitor. Use
    /// [`Self::configure_coap`] in production.
    #[cfg(test)]
    ...
    /// Test-only mutable accessor for the BLE monitor. Use
    /// [`Self::configure_ble`] in production.
    #[cfg(test)]
    ...
    /// Test-only mutable accessor for the Zigbee monitor. Use
    /// [`Self::configure_zigbee`] in production.
    #[cfg(test)]
    ...
    /// Test-only mutable accessor for the `LoRa` monitor. Use
    /// [`Self::configure_lora`] in production.
    #[cfg(test)]
    ...
    /// Test-only mutable accessor for the Modbus monitor. Use
    /// [`Self::configure_modbus`] in production.
    #[cfg(test)]
    ...
    ///
    /// O(1): equal to `min(recent_alert_count, MAX_RECENT_ALERTS)` since
    /// `store_recent_alert` writes a real alert (id != 0) on every call and
    /// the ring buffer wraps deterministically. Avoids scanning all 32 slots
    /// on every health-check tick.
    pub fn recent_alert_count_valid(&self) -> usize {
        (self.recent_alert_count as usize).min(MAX_RECENT_ALERTS)
    }
    ...
    pub fn drain_recent_alerts_into(&mut self, buf: &mut [SecurityAlert]) -> usize {
        ...
        for (i, slot) in buf.iter_mut().enumerate().take(to_copy) {
            // `MAX_RECENT_ALERTS` is asserted to be a power of two at
            // compile time, so bitmask is equivalent to `%` and avoids a
            // division on the hot path.
            let src_idx = (start + i) & (MAX_RECENT_ALERTS - 1);
    ...
    /// Test-only convenience that copies the recent-alert ring buffer onto
    /// the stack and clears it.
    ///
    /// Production code must use [`Self::drain_recent_alerts_into`] to avoid
    /// the large stack allocation. Hidden behind `#[cfg(test)]` so it is
    /// not part of the v1.0 stable surface.
    #[cfg(test)]
    ...
    fn compute_health(
        ...
        // Use `saturating_mul` instead of `<<` so the upper bits cannot be
        // lost at extreme counter values. With `HEALTH_MIN_MESSAGES = 100`
        // this is defense-in-depth only — overflow is unreachable in
        // practice — but it costs nothing on the integer path.
        if total_inspected >= HEALTH_MIN_MESSAGES
            && total_alerts.saturating_mul(2) > total_inspected
        {
            SubsystemStatus::Degraded
        } else if matches!(current, SubsystemStatus::Degraded)
            && total_alerts.saturating_mul(4) < total_inspected
        {
...
mod tests {
    ...
    fn embedded_coap_amplification() {
        ...
        let peer = vs_coap_monitor::CoapPeer::v4([192, 168, 1, 50], 5683);
        ...
        msg.token[0] = 0x01;
        let _ = shield.submit_coap_message_with_peer(&mut msg, &peer, 1000);

        let alert = shield.check_coap_amplification_with_peer(42, &[0x01], &peer, 500, 2000);
    ...
    fn embedded_coap_amplification_with_token() {
        ...
        let alert = shield.check_coap_amplification_with_peer(42, &[0xAB, 0xCD], &peer, 500, 2000);
        assert!(alert.is_some());

        // Wrong token does not match.
        let alert2 = shield.check_coap_amplification_with_peer(42, &[0xFF, 0xFF], &peer, 500, 3000);
    ...
    fn check_coap_amplification_zero_timestamp() {
        let mut shield = make_shield();
        let peer = vs_coap_monitor::CoapPeer::v4([10, 0, 0, 1], 5683);
        let r = shield.check_coap_amplification_with_peer(1, &[0x01], &peer, 100, 0);
    ...
    fn check_coap_amplification_after_shutdown() {
        ...
        shield.shutdown();
        let peer = vs_coap_monitor::CoapPeer::v4([10, 0, 0, 1], 5683);
        let r = shield.check_coap_amplification_with_peer(1, &[0x01], &peer, 100, 1_000_000);
