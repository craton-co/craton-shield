...
> the packed `VS_ABI_VERSION` constant (see [`vs_auto.h`](vs_auto.h) and
> [`ABI.md`](../../ABI.md) at the workspace root). A patch-level Rust
> release MAY bump `VS_ABI_VERSION` (e.g. fixing a field-layout bug);
> downstream C consumers MUST query `vs_auto_abi_version()` at init and
