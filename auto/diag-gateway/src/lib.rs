...
pub enum BlockReason {
    ...
    /// Generic programming failure (used for crypto-provider faults such
    /// as RNG or HMAC errors). Mapped to UDS NRC 0x72 to avoid leaking
    /// information about which subsystem failed.
...
impl BlockReason {
    ...
    #[must_use]
    pub fn nrc(self) -> u8 {
        // Ordering note: `SecurityAccessDenied` is intentionally tied to
        // `Unauthorized` (both map to NRC 0x33) so a client cannot tell
        // whether the session is unauthenticated or merely under-leveled.
        // `GeneralProgrammingFailure` is grouped with `SessionsFull` under
        // NRC 0x72 to avoid leaking the failure subsystem.
        match self {
            ...
            BlockReason::SessionsFull | BlockReason::GeneralProgrammingFailure => {
...
pub struct LockoutEntry {
    /// Tester address being tracked.
    pub tester_address: u16,
    /// Number of consecutive `SecurityAccess` failures observed.
    pub fail_count: u8,
    /// Absolute timestamp (microseconds) at which the lockout expires.
    pub locked_until_us: u64,
    /// Number of times lockout has been triggered (for exponential backoff).
    pub lockout_generation: u8,
    /// `true` if this slot holds a live tracking entry; `false` if free.
    pub active: bool,
}
...
impl<C: CryptoProvider> DiagGateway<C> {
    ...
    pub fn restore_lockouts_from(&mut self, entries: &[LockoutEntry]) -> usize {
        ...
        'outer: for src in entries {
            ...
            // First pass: if an active slot for this tester already exists,
            // overwrite it (prevents duplicate entries for the same tester
            // when an inactive slot precedes the matching active slot).
            for slot in &mut self.lockouts {
                if slot.active && slot.tester_address == src.tester_address {
                    *slot = *src;
                    restored += 1;
                    continue 'outer;
                }
            }
            // Second pass: pick any inactive slot.
            for slot in &mut self.lockouts {
                if !slot.active {
                    *slot = *src;
                    restored += 1;
            ...
            // Third pass: no free slot — evict the entry with the oldest
            // (smallest) `locked_until_us`. Skip if no active entries.
            let mut evict_idx: Option<usize> = None;
            let mut oldest = u64::MAX;
            for (i, slot) in self.lockouts.iter().enumerate() {
                if slot.active && slot.locked_until_us <= oldest {
                    oldest = slot.locked_until_us;
                    evict_idx = Some(i);
                }
            }
    ...
    pub fn receive_uds_request(
        ...
        // Enforce timestamp monotonicity FIRST, before mutating any state.
        // Non-monotonic timestamps could allow attackers to bypass lockout
        // durations by supplying a timestamp in the past — if we cleared
        // `recently_expired` or advanced `last_timestamp_us` before this
        // check, a regressing-timestamp request could still corrupt state.
        if ts_us < self.last_timestamp_us {
            self.audit.record(tester_addr, sid, DECISION_BLOCK, ts_us);
            return DiagDecision::Block(BlockReason::PolicyDenied);
        }

        // Lockout check BEFORE any state mutation. A locked-out tester
        // must not be able to clear another tester's `recently_expired`
        // slot or advance `last_timestamp_us` simply by being scheduled.
        let locked = self.is_locked_out(tester_addr, ts_us);
        if locked {
            self.audit.record(tester_addr, sid, DECISION_BLOCK, ts_us);
            return DiagDecision::Block(BlockReason::LockedOut);
        }

        // Now safe to clear `recently_expired` (via expire_sessions) and
        // advance the monotonic timestamp.
        self.expire_sessions(ts_us);
        self.last_timestamp_us = ts_us;

        let decision = self.evaluate_unlocked(tester_addr, sid, payload, ts_us);
    ...
    /// Evaluate a request once the lockout check has already been performed.
    ///
    /// Callers in `receive_uds_request` invoke this after `is_locked_out`
    /// has already returned `false`, so we skip a second linear scan.
    /// Internal helpers (`get_or_create_session`) still call `is_locked_out`
    /// directly because they're reachable from other entry points.
    fn evaluate_unlocked(
        ...
        //
        // SECURITY NOTE: `SID_DIAGNOSTIC_SESSION_CONTROL` (0x10) is exempted
        // here because switching back to the default session must always be
        // permitted (it lowers the security level rather than raising it).
        // However, a hostile tester can also use 0x10 sub-function 0x02
        // (programming session) to elevate without re-authenticating —
        // high-security deployments SHOULD therefore call
        // `policy.require_auth_for_sid(0x10)` so that programming-session
        // transitions require an authenticated session first.
        if sid != SID_SECURITY_ACCESS && sid != SID_DIAGNOSTIC_SESSION_CONTROL {
    ...
    fn handle_seed_request(
        ...
        // Generate a random seed. On RNG failure we return
        // `GeneralProgrammingFailure` rather than `PolicyDenied` so the
        // tester cannot distinguish "policy refused" from "crypto provider
        // unhealthy" — this avoids leaking liveness signals about the
        // underlying HSM/TRNG to a probing attacker.
        let mut seed = [0u8; 16];
    ...
    fn get_or_create_session(&mut self, tester_addr: u16, ts_us: u64) -> Option<usize> {
        ...
        // NOTE: The caller (`receive_uds_request` → `evaluate_unlocked`)
        // has already verified that `tester_addr` is not locked out, so
        // we skip the redundant scan here. `debug_assert!` keeps the
        // invariant explicit in dev builds without a release-mode cost.
        debug_assert!(
            !self.is_locked_out(tester_addr, ts_us),
            "get_or_create_session must only be called after the caller has \
             cleared the lockout check"
        );
    ...
    fn expire_sessions(&mut self, ts_us: u64) {
        // Fast path: if no session can possibly be expired yet, skip
        // both the scan and the `recently_expired` reset. The reset only
        // matters when we are about to record fresh expirations.
        if ts_us < self.nearest_expiry_us {
            return;
        }

        // Slow path: clear stale `recently_expired` entries first so that
        // any new expirations recorded below are not commingled with the
        // previous tick's results.
    ...
    /// Record a `SecurityAccess` failure for a tester and (if the threshold
    /// is reached) compute the new lockout expiry timestamp.
    ///
    /// # Security: trusted-timestamp requirement
    ///
    /// The caller MUST source `ts_us` from a trusted monotonic clock. The
    /// computed lockout expiry is `ts_us.saturating_add(duration)`. If an
    /// adversary supplies a `ts_us` value close to `u64::MAX`, the
    /// saturation can produce a permanent or near-permanent lockout —
    /// effectively self-DoS-ing the legitimate tester (or any other tester
    /// whose slot is collated against this expiry). Never derive `ts_us`
