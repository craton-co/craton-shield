...
pub fn extract_signal_raw(
    ...
    // (mirrors the clamp in `extract_signal`). Without this clamp, the
    // byte-aligned fast path in `extract_raw_le` would index past
    // `frame_data.len()` and panic — a DoS through a public API of a
    // safety-critical IDS.
...
fn extract_raw_le(frame_data: &[u8], data_len: usize, def: &SignalDefinition) -> Option<u64> {
    ...
    // Defense-in-depth: callers should have clamped `data_len` to
    // `frame_data.len()` already. Guard against a stale or misused
    // `data_len` so subsequent indexing cannot panic.
    debug_assert!(data_len <= frame_data.len());
    let data_len = data_len.min(frame_data.len());
    ...
    if start % 8 == 0 && length % 8 == 0 {
        ...
        // Defense-in-depth: callers should have clamped `data_len` to
        // `frame_data.len()`, and the `required_bytes > data_len` check
        // above should have prevented overrun. Guard the slice index
        // explicitly so a logic bug or untrusted call site cannot panic.
        debug_assert!(start_byte + num_bytes <= frame_data.len());
        if start_byte + num_bytes > frame_data.len() {
            return None;
        }
        let mut value: u64 = 0;
        let mut i = 0;
...
///
/// Includes a fast path for byte-aligned BE signals (`start_bit % 8 == 7`
/// and `length % 8 == 0`), which is the common DBC convention for Motorola
/// signals starting on a byte boundary. The fast path reads whole bytes
/// directly, avoiding the per-bit loop.
fn extract_raw_be(frame_data: &[u8], data_len: usize, def: &SignalDefinition) -> Option<u64> {
    ...
    // Defense-in-depth: callers should have clamped `data_len` to
    // `frame_data.len()` already. Guard against a stale or misused
    // `data_len` so subsequent indexing cannot panic. The per-iteration
    // `byte_idx >= data_len` check below still bounds individual reads,
    // but this clamp ensures the bound is sound.
    debug_assert!(data_len <= frame_data.len());
    let data_len = data_len.min(frame_data.len());

    // Fast path: byte-aligned Motorola signals (`start_bit % 8 == 7` puts
    // the MSB at bit 7 of a byte and `length % 8 == 0` ends on a byte
    // boundary). Read `num_bytes` consecutive bytes starting at
    // `start / 8`, with the first byte being most significant.
    if start % 8 == 7 && length % 8 == 0 && length > 0 {
        let start_byte = start / 8;
        let num_bytes = length / 8;
        let end_byte = start_byte.saturating_add(num_bytes);
        if end_byte > data_len {
            return None;
        }
        // Defense-in-depth: callers should have clamped `data_len` to
        // `frame_data.len()`, and the `end_byte > data_len` check above
        // should have prevented overrun. Guard the slice index explicitly
        // so a logic bug or untrusted call site cannot panic.
        debug_assert!(end_byte <= frame_data.len());
        ...
        let mut i = 0;
        while i < num_bytes {
            value = (value << 8) | (frame_data[start_byte + i] as u64);
            i += 1;
        }
        return Some(value);
    }
...
/// Signal-level CAN intrusion detection engine.
///
/// Tracks `MAX_SIGNALS` extracted signal values plus per-signal EWMA/Markov
/// anomaly detectors. Built for `no_std` deployment with no heap allocation.
...
impl SignalIdsEngine {
    ...
    /// Create a new signal-IDS engine with the given EWMA `alpha` and z-score
    /// anomaly threshold. Returns `Err(InvalidInput)` if either parameter is
    /// out of range.
    ...
    /// * `scale` or `offset` is not finite (NaN or +/- infinity) — would
    ///   poison every subsequent EWMA update with non-finite physical values.
    /// * `is_multiplexor` is `true` and `bit_length > 16` — the engine
    ///   truncates the raw multiplexor value to `u16`, so wider multiplexors
    ///   would silently collide on the low 16 bits and route frames to the
    ///   wrong multiplexed signal set.
    /// * `signed` is `true` and `bit_length < 2` — a one-bit signed signal
    ...
    /// # Performance
    ///
    /// Each call rebuilds the internal CAN-ID fast-path index in `O(n^2)`
    /// (bounded by `MAX_SIGNALS = 64`, so at most 4096 comparisons). When
    /// inserting many signals at once — e.g. at ECU startup from a DBC
    /// description — prefer [`Self::define_signals_batch`], which rebuilds
    /// the index a single time after all signals are inserted.
    pub fn define_signal(&mut self, def: SignalDefinition) -> Result<usize, VsError> {
        ...
        if def.signed && def.bit_length < 2 {
            return Err(VsError::InvalidInput);
        }
    ...
    /// Returns `VsError::InvalidInput` if any definition has non-finite
    /// `scale`/`offset`, is a multiplexor with `bit_length > 16`, or is
    /// `signed` with `bit_length < 2`. See [`Self::define_signal`] for the
    /// rationale behind each rejection. Validation runs over the whole batch
    /// before any state mutation, so a failing batch leaves the engine
    /// unchanged.
            ...
            while i < self.signal_count && self.definitions[i].can_id == frame.id {
                if self.definitions[i].is_multiplexor {
                    if let Some(raw) = extract_raw_bits(&frame.data, data_len, &self.definitions[i])
                    {
                        ...
                        // fits in `u16`. Guard against logic bugs that would
                        // silently truncate to a colliding low-16 value.
                        debug_assert!(u16::try_from(raw).is_ok());
    ...
    pub fn remove_signal(&mut self, slot_index: u16) -> Result<(), VsError> {
        ...
        // `saturating_sub` is defensive: the `signal_count == 0` case is
        // already excluded by the slot-map lookup above (which returns
        // `Err` on empty), but guard against future changes that could
        // reach this code with `signal_count == 0` and underflow.
        for i in idx..self.signal_count.saturating_sub(1) {
            self.definitions[i] = self.definitions[i + 1];
            self.detectors[i] = self.detectors[i + 1];
            self.slot_map[i] = self.slot_map[i + 1];
        }
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Regression tests for the `extract_signal_raw` data_len clamp.
    //
    // Before the fix, passing a `data_len` larger than `frame_data.len()`
    // to the public `extract_signal_raw` would panic in the byte-aligned
    // fast path of `extract_raw_le`, giving a DoS via a public API of a
    // safety-critical IDS.
    // -----------------------------------------------------------------------

    #[test]
    fn extract_signal_raw_data_len_exceeds_slice_no_panic_le_byte_aligned() {
        // Byte-aligned 8-bit LE signal. Caller lies about data_len.
        // Pre-fix: panicked in the fast-path indexing `frame_data[0..8]`
        ...
        // Single-byte frame, but caller claims 100 bytes available.
        let result = extract_signal_raw(&[0xAB], 100, &def);
        // After clamp, only 1 byte is available — required_bytes (8) > data_len (1)
        // so the function reports "no extraction possible" rather than panicking.
        assert_eq!(result, None);
    }

    #[test]
    fn extract_signal_raw_data_len_exceeds_slice_no_panic_le_8bit() {
        // 8-bit LE signal that fits in a 1-byte frame. Caller still
    ...
    #[test]
    fn extract_signal_raw_data_len_exceeds_slice_no_panic_be() {
        ...
        // Only 1 byte present, but caller says 100.
        let result = extract_signal_raw(&[0xAB], 100, &def);
        // Signal needs 2 bytes; only 1 is available — no extraction.
        assert_eq!(result, None);
    }

    #[test]
    fn extract_signal_raw_short_frame_returns_none_not_panic() {
        // Frame shorter than the signal's required bytes returns the
        ...
        // 1-byte frame, honest data_len.
        assert_eq!(extract_signal_raw(&[0xAB], 1, &def), None);
        // 1-byte frame, dishonest data_len.
        assert_eq!(extract_signal_raw(&[0xAB], 8, &def), None);
        // Empty frame.
        assert_eq!(extract_signal_raw(&[], 0, &def), None);
        assert_eq!(extract_signal_raw(&[], 100, &def), None);
    }

    // -----------------------------------------------------------------------
    // Regression tests for `define_signal` / `define_signals_batch` input
    // validation:
    //
    // * non-finite `scale` / `offset` (would poison the EWMA detector with
    //   NaN/Infinity physical values),
    // * `is_multiplexor` with `bit_length > 16` (engine truncates mux raw
    //   to `u16`, so wider multiplexors silently collide),
    // * `signed` with `bit_length < 2` (sign extension of a 1-bit value
    //   produces flip-flopping +0.0/-1.0 outputs that defeat anomaly
    //   detection).
    // -----------------------------------------------------------------------

    #[test]
    fn define_signal_rejects_nan_scale() {
        let mut engine = default_engine();
        let def = SignalDefinition {
            can_id: 0x100,
            scale: f32::NAN,
    ...
    #[test]
    fn define_signal_rejects_infinite_scale() {
        let mut engine = default_engine();
        let def = SignalDefinition {
            can_id: 0x100,
            scale: f32::INFINITY,
    ...
    #[test]
    fn define_signal_rejects_nan_offset() {
        let mut engine = default_engine();
        let def = SignalDefinition {
            can_id: 0x100,
            offset: f32::NAN,
    ...
    #[test]
    fn define_signal_rejects_infinite_offset() {
        let mut engine = default_engine();
        let def = SignalDefinition {
            can_id: 0x100,
            offset: f32::INFINITY,
    ...
    #[test]
    fn define_signal_accepts_finite_scale_and_offset() {
        // Sanity check: legitimate finite scale/offset (including 0 and
        // negatives) still succeed after the new finite check.
        let mut engine = default_engine();
    ...
    #[test]
    fn define_signal_rejects_multiplexor_wider_than_16_bits() {
        let mut engine = default_engine();
        // 17-bit multiplexor: would be truncated to u16 in process_frame,
        // silently aliasing distinct mux selectors onto the same low-16-bit
        // value.
        let def = SignalDefinition {
            can_id: 0x100,
            bit_length: 17,
    ...
    #[test]
    fn define_signal_accepts_multiplexor_up_to_16_bits() {
        // Boundary: 16-bit multiplexor must still be accepted.
        let mut engine = default_engine();
        let def = SignalDefinition {
            can_id: 0x100,
            bit_length: 16,
    ...
    #[test]
    fn define_signal_rejects_signed_one_bit() {
        let mut engine = default_engine();
        let def = SignalDefinition {
            can_id: 0x100,
            bit_length: 1,
    ...
    #[test]
    fn define_signal_accepts_signed_two_bit_and_unsigned_one_bit() {
        // Boundary: 2-bit signed is the smallest meaningful signed signal
        // (one sign + one magnitude), and unsigned 1-bit flags remain valid.
        let mut engine = default_engine();
        ...
        assert!(engine.define_signal(one_bit_unsigned).is_ok());
        assert_eq!(engine.signal_count(), 2);
    }

    #[test]
    fn define_signals_batch_rejects_nan_scale_and_leaves_state_unchanged() {
        let mut engine = default_engine();
        let defs = [
            SignalDefinition {
                can_id: 0x100,
        ...
        assert_eq!(
            engine.define_signals_batch(&defs),
            Err(VsError::InvalidInput)
        );
        // Pre-validation: no partial state.
        assert_eq!(engine.signal_count(), 0);
    }

    #[test]
    fn define_signals_batch_rejects_infinite_offset() {
        let mut engine = default_engine();
        let defs = [SignalDefinition {
            can_id: 0x100,
            offset: f32::INFINITY,
    ...
    #[test]
    fn define_signals_batch_rejects_wide_multiplexor() {
        let mut engine = default_engine();
        let defs = [SignalDefinition {
            can_id: 0x100,
            bit_length: 17,
    ...
    #[test]
    fn define_signals_batch_rejects_signed_one_bit() {
        let mut engine = default_engine();
        let defs = [SignalDefinition {
            can_id: 0x100,
            bit_length: 1,
    ...
    #[test]
    fn define_signals_batch_accepts_legit_defs() {
        // Mixed-validity batch: every entry is legitimate (boundary values
        // included) so the batch must succeed.
        let mut engine = default_engine();
        let defs = [
            SignalDefinition {
                can_id: 0x100,
