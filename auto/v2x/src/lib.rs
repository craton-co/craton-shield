...
//! V2X communication security for IEEE 1609.2 environments.
//!
//! Provides certificate chain validation, ECDSA P-256 signature verification, replay
//! detection with bloom filter, BSM/CAM plausibility checks, CRL management, PSID
//! policy enforcement, geographic region filtering, and misbehavior detection.
//!
//! # Feature flags
//!
...
impl<C: CryptoProvider> V2xValidator<C> {
    ...
    fn compute_digest(&self, msg: &V2xMessage) -> Result<[u8; 32], VsError> {
        ...
        // A future improvement would be to add a streaming hash trait
        // extension (e.g. `Sha256Stream { update(&[u8]), finish(&mut
        // [u8;32]) }`) that allows hashing header and payload separately
...
impl TrustStore {
    ...
    fn verify_chain_structural(
        ...
        let mut i = 1;
        while i < chain.len() - 1 {
            if chain[i].cert_type != CertificateType::Intermediate {
                return Err(VsError::InvalidInput);
            }
            i += 1;
        }

        // Verify issuer-subject linkage and time validity.
        i = 0;
        while i < chain.len() {
            // Time validity.
...
pub struct MisbehaviorDetector {
    ...
    /// Index of the least-recently-seen sender. Updated on each
    /// `check_sender` call to avoid an O(n) linear scan when evicting
    /// the oldest sender in `find_or_create_sender`.
    lru_index: usize,
}
...
impl MisbehaviorDetector {
    ...
    pub fn check_sender(
        ...
        // Uses `>=` so that messages arriving at the same microsecond
        // (elapsed_us == 0) are also rate-limited, preventing replay-like
        // attacks with non-increasing timestamps.
        if profile.message_count > 0 && current_time_us >= profile.last_seen_us {
            ...
            if elapsed_us < 1_000_000 {
                // Within one second — check rate.
                let mut max_msgs = (self.max_messages_per_second as u64) * elapsed_us / 1_000_000;
                if elapsed_us == 0 {
                    // Allow the first message in the same microsecond but reject subsequent ones.
                    // This prevents false positives from legitimate high-rate V2X environments
                    // while still limiting within-tick duplicates.
                    max_msgs = 1;
                }
                if profile.message_count >= max_msgs {
                    // Too fast.
        ...
        // Update LRU tracking: if we just updated the current LRU sender,
        // rescan to find the new LRU. Otherwise, check if this sender's
        // previous timestamp was the LRU and update accordingly.
    ...
    fn find_or_create_sender(&mut self, hash: [u8; 16]) -> usize {
        ...
        // All slots full — evict the least-recently-seen sender (tracked
        // by lru_index) to prevent a DoS where an attacker floods with
        // unique keys to exhaust all slots and block legitimate senders.
        let evict_idx = self.lru_index;
        self.init_sender(evict_idx, hash);
        // After eviction the new sender has last_seen_us = 0, so it
        // remains the LRU until update_lru_index is called.
        evict_idx
    }
    ...
    ///
    /// This is intentionally O(n) to maintain constant-time behavior —
    /// an incremental tracker would leak information about which slot
    /// was accessed through timing variations.
...
mod tests {
    ...
    fn misbehavior_rate_limiting() {
        ...
        // First message: accepted (creates sender profile).
        assert!(detector.check_sender(&msg, 1_000_000, hash).is_ok());
        assert_eq!(detector.sender_count(), 1);

        // Second message very shortly after (10 us) — rate limited.
        assert_eq!(
            detector.check_sender(&msg, 1_000_010, hash),
            Err(VsError::PolicyViolation)
        );
    }
    ...
    fn misbehavior_suspicious_sender() {
        ...
        // Simulate rapid messages that get rate-limited (rejection).
        let _ = detector.check_sender(&msg, 1_000_010, hash);
        let _ = detector.check_sender(&msg, 1_000_020, hash);
        let _ = detector.check_sender(&msg, 1_000_030, hash);
