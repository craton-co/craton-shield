...
//! # Stack usage
//!
//! [`AutomotiveShield`] is **very large on the stack** (>20 KiB) due to the
...
//!
//! Tests use an 8 MiB stack (see `big_stack` helper) to accommodate this.
...
/// Permissive OTA signature verifier that accepts all signatures.
///
/// Only hash-based integrity checking is performed. This is intended
/// **only** for development and testing — production builds **must**
/// replace this with a real implementation backed by a signing key.
///
/// # Security warning
///
/// Using this verifier disables OTA code signing verification. An attacker
/// with network access to the OTA update path could install arbitrary
/// firmware on the ECU.
#[cfg(any(debug_assertions, test, feature = "stub"))]
#[deprecated(
    since = "0.8.0",
    note = "NoOpOtaSigner disables OTA signature verification. \
            Provide a real OtaSignatureVerifier for production."
)]
pub struct NoOpOtaSigner;

#[cfg(any(debug_assertions, test, feature = "stub"))]
#[allow(deprecated)]
impl OtaSignatureVerifier for NoOpOtaSigner {
    fn verify_ota_signature(
        &self,
        data: &[u8],
        signature: &[u8],
        ...
        // Fail-closed: reject all OTA signatures in stub mode to prevent
        // accidental acceptance of unsigned firmware. This matches the
        // fail-closed pattern used by StubCryptoProvider::hmac_sha256 and
        // NoOpSomeIpAuth. Tests that need OTA verification to succeed
        // should provide a real (or test-specific) OtaSignatureVerifier.
        let _ = (data, signature);
        Ok(false)
    }
}
...
// Compile-time assertion: when heap-subsystems is *disabled*,
// `AutomotiveShield` lives entirely on the stack. Ensure callers are aware
// of the size by failing compilation if it grows beyond 128 KiB (a
// reasonable ceiling; the current expected size is ~20-40 KiB depending on
// the CryptoProvider).
//
// When `heap-subsystems` is enabled the V2X validator and DiagGateway are
// boxed, so the stack footprint is much smaller and this check is skipped.
#[cfg(not(feature = "heap-subsystems"))]
const _: () = {
    // We cannot name the concrete type without a CryptoProvider, so this
    // assertion is intentionally left as a documentation marker. Crate-level
...
impl<C: CryptoProvider + Clone> AutomotiveShield<C> {
    ...
    /// code should use [`AutomotiveShield::init`] and handle the error.
    ...
    pub fn submit_can_frame(&mut self, frame: &CanFrame, ts_us: u64) -> Result<(), VsError> {
        ...
        if result.anomaly_count > 0 {
            // Compute a cryptographic payload fingerprint for alert tracing
            // using the platform's SHA-256 provider. Fail-closed: if the
            // crypto provider is unavailable, use all-zeros hash and set
            // the subsystem status to Degraded to surface the issue rather
            // than silently producing a weak fingerprint.
            let mut hash_bytes = [0u8; 32];
            let payload_len = (frame.dlc as usize).min(64);
            if self
                .core
                .crypto()
                .sha256(&frame.data[..payload_len], &mut hash_bytes)
                .is_err()
            {
                self.signal_ids_status = SubsystemStatus::Degraded;
            } else if self.signal_ids_status == SubsystemStatus::Degraded {
                self.signal_ids_status = SubsystemStatus::Ready;
            }
            ...
            let alert = build_alert(
                ts_us,
                source,
                hash_mix_from_bytes(&hash_bytes),
            ...
            if port == DOIP_PORT && pkt.payload.len() > 8 {
                ...
                if version != !inverse {
                    ...
                    if self
                        .core
                        .crypto()
                        .sha256(&pkt.payload[..hash_len], &mut hash_bytes)
                        .is_err()
                    {
                        self.signal_ids_status = SubsystemStatus::Degraded;
                    }
    ...
    pub fn validate_v2x_message(
        ...
        if result.is_err() {
            ...
            if self
                .core
                .crypto()
                .sha256(&msg.signer_public_key, &mut hash_bytes)
                .is_err()
            {
                self.signal_ids_status = SubsystemStatus::Degraded;
            }
    ...
    pub fn submit_lin_frame(
        ...
        if payload.is_empty() || payload.len() > 8 {
            ...
            if self
                .core
                .crypto()
                .sha256(&hash_input[..input_len], &mut hash_bytes)
                .is_err()
            {
                self.signal_ids_status = SubsystemStatus::Degraded;
            }
        ...
        if result.anomaly_count > 0 {
            let mut hash_bytes = [0u8; 32];
            if self.core.crypto().sha256(payload, &mut hash_bytes).is_err() {
                self.signal_ids_status = SubsystemStatus::Degraded;
            }
    ...
    pub fn submit_flexray_frame(
        ...
        if payload.is_empty() || payload.len() > 254 {
            ...
            if self
                .core
                .crypto()
                .sha256(&payload[..hash_len], &mut hash_bytes)
                .is_err()
            {
                self.signal_ids_status = SubsystemStatus::Degraded;
            }
        ...
        match sig_ok {
            ...
            Ok(false) => {
                ...
                if self
                    .core
                    .crypto()
                    .sha256(&manifest_data[..hash_len], &mut hash_bytes)
                    .is_err()
                {
                    self.signal_ids_status = SubsystemStatus::Degraded;
                }
