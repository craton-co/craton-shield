...
impl<C: SecOcCrypto> SecOcManager<C> {
    ...
    pub fn verify_rx(&mut self, frame: &RawCanFrame, now_us: u64) -> SecOcVerifyResult {
        ...
        // Decode freshness value (big-endian).
        // Fast path for common freshness lengths (4 and 8 bytes) using
        // native byte-order conversion; fallback to byte loop otherwise.
        let fv: u64 = match fv_len {
            8 => {
                let mut buf = [0u8; 8];
                buf.copy_from_slice(fv_bytes);
        ...
        // Check freshness monotonicity. The incoming freshness value must
        // strictly exceed the stored counter. There is no special-case for
        // counter == 0: initial state requires fv > 0 to prevent replay of
        // a pre-reboot frame with fv=0.
        ...
        // Build verification input: auth_data || freshness_value.
        // CAN-FD max is 64 bytes payload; freshness max 8 bytes = 72 max.
        // Use a buffer that can hold the maximum CAN-FD frame + freshness.
        let mut verify_buf = [0u8; 72];
        if auth_data_len + fv_len > verify_buf.len() {
            self.verify_fail_count = self.verify_fail_count.saturating_add(1);
            return SecOcVerifyResult::MacMismatch;
        }
        verify_buf[..auth_data_len].copy_from_slice(auth_data);
        verify_buf[auth_data_len..auth_data_len + fv_len].copy_from_slice(fv_bytes);

        let mac_ok = match self.crypto.verify_mac(
            pdu.key_id,
            &verify_buf[..auth_data_len + fv_len],
            received_mac,
            mac_len,
        ) {
            Ok(true) => true,
        ...
        // Update freshness state on success.
        self.freshness[slot].counter = fv;
    ...
    pub fn prepare_tx(
        ...
        self.freshness[slot].last_seen_us = now_us;
        let fv = self.freshness[slot].counter;

        // Encode freshness value (big-endian) into frame.
        for i in 0..fv_len {
            let shift = (fv_len - 1 - i) * 8;
            frame.data[auth_data_len + i] = (fv >> shift) as u8;
        }

        // Build MAC input: auth_data || freshness_value.
        let mut mac_input = [0u8; 72];
        let input_len = auth_data_len + fv_len;
        if input_len > mac_input.len() {
            return Err(VsError::ResourceExhausted);
        }
        mac_input[..input_len].copy_from_slice(&frame.data[..input_len]);
    ...
    pub fn active_pdu_count(&self) -> usize {
        self.pdus.iter().filter(|p| p.active).count()
    }
...
impl IdsmReporter {
    ...
    pub fn report_alert(&mut self, alert: &SecurityAlert) -> Result<u64, VsError> {
        ...
        context.copy_from_slice(&alert.payload_hash.as_bytes()[..8]);
...
///
/// The default implementation (`NoOpSomeIpAuth`) performs no authentication,
/// matching the current AUTOSAR Classic behavior where SOME/IP relies on
/// network-level trust. Production AUTOSAR Adaptive deployments should
/// provide a real implementation backed by the platform's TLS stack
...
pub trait SomeIpAuthProvider {
    ...
    /// authentication fails, or `Err` on internal error.
    ///
    /// Default: **fail-closed** — rejects all messages until a real
    /// implementation is provided. This ensures that forgetting to wire up
    /// a TLS/DTLS backend does not silently disable authentication.
    ...
    ///
    /// Returns `Ok(true)` if the session is authenticated.
    ///
    /// Default: **fail-closed** — rejects all calls.
...
/// Fail-closed SOME/IP authentication stub.
///
/// Rejects **all** authentication attempts with `Err(VsError::NotInitialized)`.
/// This ensures that forgetting to wire up a real TLS/DTLS backend does not
/// silently disable authentication — every call fails until replaced.
///
/// # Security note
///
/// This is the correct default for any deployment that has not yet integrated
/// a real [`SomeIpAuthProvider`]. Production AUTOSAR Adaptive deployments
/// **must** replace this with a TLS/DTLS-backed implementation.
/// See `THREAT_MODEL.md` § T5 for spoofing risks.
#[deprecated(
    since = "0.8.0",
    note = "NoOpSomeIpAuth rejects all SOME/IP authentication. \
            Provide a real SomeIpAuthProvider backed by TLS/DTLS for production."
)]
pub struct NoOpSomeIpAuth;

#[allow(deprecated)]
impl SomeIpAuthProvider for NoOpSomeIpAuth {
    fn verify_sd_message(
        &self,
        _source_ip: u32,
        _service_id: u16,
...
mod tests {
    ...
    // ======================== V6 — NoOpSomeIpAuth fail-closed ========================

    #[test]
    #[allow(deprecated)]
    fn noop_someip_auth_rejects_sd_message() {
        let auth = NoOpSomeIpAuth;
        let result = auth.verify_sd_message(0x0A00_0001, 0x1234, 0x0001, &[]);
        assert_eq!(result, Err(VsError::NotInitialized));
    }

    #[test]
    #[allow(deprecated)]
    fn noop_someip_auth_rejects_method_call() {
        let auth = NoOpSomeIpAuth;
        let result = auth.verify_method_call(0x0A00_0001, 0x1234, 0x0001, 0x0001, &[]);
        assert_eq!(result, Err(VsError::NotInitialized));
    }

    #[test]
    #[allow(deprecated)]
    fn noop_someip_auth_session_not_established() {
        let auth = NoOpSomeIpAuth;
        assert!(!auth.is_session_established(0x0A00_0001));
    }
