...
crates.io publish-readiness pass: doc corrections, metadata fixes, dependency
hygiene, and deprecation-policy reconcile across the 49-crate workspace.
...
### Security
- Closed VULN-08-class LRU bypass in `profinet-monitor` and `mqtt-monitor`
  `rate_check` paths that allowed attackers to evict legitimate state by
  cycling rate-limit keys.
- Removed `unsafe { NonZeroU32::new_unchecked }` from `core/crypto` to restore
  workspace-wide `#![forbid(unsafe_code)]` invariant outside of `vs-ffi` and
  `vs-hal-linux`.
...
- Architecture document clarified as describing core layer within the 49-crate multi-domain workspace.
...
- `evidence-envelope` schema-version round-trip data loss: component counts
  greater than 255 were silently truncated on serialization; widened the field
  and added round-trip coverage.
- `Cargo.toml` `description` sharpened to reflect the non-cryptographic scope
  of the affected crates (avoids implying FIPS-grade primitives where none
  are provided).
- Documented crate count corrected from 47 to 49 across `README.md`,
  `ARCHITECTURE.md`, `ROADMAP.md`, `RELEASING.md`, and the
  `core/docs/migration-guide-0.6-to-0.7.md`.

### Changed
- Removed workspace-level `documentation` field; added per-crate
  `readme = "README.md"` to the 8 crates that ship a crate-local README, so
  crates.io renders the right landing page for each.
- Reconciled `DEPRECATION.md` and `core/docs/api-stability.md` so the pre-1.0
  vs. post-1.0 stability framing matches (pre-1.0: minor versions may break
  with migration notes; post-1.0: deprecation cycle as documented).
...
- 49-crate workspace spanning four security domains: Core (23 crates),
...
[Unreleased]: https://github.com/craton-co/craton-shield/compare/v0.7.1...HEAD
[0.7.1]: https://github.com/craton-co/craton-shield/compare/v0.7.0...v0.7.1
